...
### 批次六十（2026-09-09 21:45，β=0.25 891 全量确认终态——calib100 假阳性被预注册机制捕获，正向信号撤回）

- **β=0.25_full 终态**（16:44，wall 10.42h，891 源，0 violation，cpu_fallback=false）：recovery 0.1210 / topk10 0.0629 / unique 0.860。
- **891 尺度 sc-hit@1 口径对位（n=214 S+ 源 vs unguided B=32 n=216）**：base support 0.2424 / sc-hit@1 0.5131；β=0.25 support 0.2402 / sc-hit@1 0.4999。**Δsc-hit@1 = −0.017 CI [−0.047,+0.012] 跨零；Δsupport −0.002 跨零**——calib100 的 +0.153 CI[+0.026,+0.291] 过门信号在 891 上**完全消失**。
- **归因（如实）**：calib100 过门 = S+ 仅 16 源的高方差点估计（下界 0.026 本就脆弱）；891 的 214 源判定显示 β=0.25 引导无真实排序增益——**批次五十七的「项目首个正向信号」撤回，降级为「calib100 子集方差假阳性」**。β=0.5（calib100 边缘 +0.101 跨零）预计同样不外推（在途，05:36 终态）。
- **机制价值**：预注册的「calib100 筛选 → 891 全量确认」两级设计按意图工作——全量确认拦截假阳性，避免在其上发射 4b（省 16-40 GPU·h）。**4b gate（watcher 1950517）将在 full891_confirm_harvest.json 落地后自动判 β=0.25 不达 B2 Δ门 → 走负结果路径自动收口（不发射 4b，amendment 诚实性条款执行）**。
- **科学终态预期**（待 β=0.5 与 gate 自动收口确认）：H3 β 杠杆的 calib100 倒 U 型是真实的（β=4 显著负在两尺度都会成立），但**弱 β 正增益不外推到全量**——与 V5 原判定（B2 引导无增益）在全量尺度一致。Phase C 最终图景 = 生成器侧四方向（q/β/扩池/组合）全负 + 诊断链完备的诚实收口。

### 批次六十附（2026-09-09 22:20，明晨终态复核清单——供任何会话一键复核）

链条状态（22:21 快照）：β=0.5_full 在途（333→推进中，ETA ~05:36）；full891 watcher（1520720）与 4b gate（1950517）每 30min 轮询正常；journal HEAD = 2ba11ce6（批 60）。

**明晨复核清单（β=0.5 终态后自动事件链的验证点）**：
1. `ls beta_full891_20260909/beta_0.5_full/guided_run_summary.json` 存在 → relay 已完成
2. `beta_full891_20260909/full891_confirm_harvest.json` 存在 → watcher 已收割（读 rows：β=0.5 的 d_sc_hit1 应跨零，复证 0.25 否证）
3. `pool256_guided_20260909/done_gate.json` 存在且 4b_launched=false → gate 已走负路径（读 gate_watch.log 尾部确认判定行 + journal 自动批）
4. git log 两个 worktree 均有自动批 commit
...
### 批次六十一（2026-09-10 05:45，Phase C 全链终态复核 + 终批——「01234 都做」收官）

- **明晨复核清单逐项验证（全部 ✓）**：① β=0.5_full 终态（wall 11.75h，recovery 0.1258，unique 0.859）→ relay 完成；② full891_confirm_harvest.json（watcher 04:35 收割）→ β=0.5 **Δsc-hit@1 −0.030 CI [−0.064, +0.005] 跨零（负点估计）**——复证 β=0.25 否证；③ done_gate.json（4b_launched=false，04:47）→ gate 走负路径，B=256 guided 未发射（amendment 诚实性条款执行，省 16-40 GPU·h）；④ 双 worktree 自动批 commit（d0afada2 + ea63a83a）。
- **Phase C 终态总表（「01234 都做」全执行完）**：

| 选项 | 终态 | 关键结果 |
|---|---|---|
| 0（amendment） | ✓ | sc-hit@1 新判据 + 分档门 + 0.35 留痕（commit 5074f8c2） |
| 1（h_bench9 探针） | ✓ | 0.0461 < V5 0.0614（3/3 V8 闭合） |
| 2（q 模型） | ✓ | AUC 0.5528 → 门 FAIL → cheap-kill（弱测量偏好信号如实入档） |
| 3（C1+C2 β sweep） | ✓ | calib100 假阳性 → 891 确认否证（0.25: −0.017 跨零 / 0.5: −0.030 跨零）；**β=4 显著负为唯一两尺度稳定信号**（H3 部分证实：强引导有害，弱引导无增益） |
| 4a（B=256 unguided） | ✓ | support 0.386（A4 高估近倍）+ polyA 死区 + sc-hit@1 稀释 |
| 4b（B=256 guided） | ✓ 负收口 | 自动 gate 判 β=0.25 不达 891 尺度 B2 Δ门 → 不发射（预注册条款执行） |

- **Phase C 科学总结论**：生成器侧四方向（q 通道/β 校准/扩池/引导×扩池组合）**在全量 891 尺度全部无正增益**——与 V5 原判一致且置信度更高（两级预注册设计 + 4 次断言拦截 + 自动化全链留痕）。calib100 倒 U 型的弱 β 正信号系子集方差假阳性（n=16 S+）。方法论遗产：fail-fast 断言链、两级筛选-确认设计、自动收割 watcher 模式、诚实性条款（负结果不追加方向）。
- **遗留（不阻塞收口，待用户拍板）**：① amendment v2（B3 绝对线 2×base 参照勘误 + 独立评估双口径正式判定数字——脚本已由并行会话落地 a62b9704）；② D2 检索条件化（q 失败 → 主选升级，spec §4-D2 条款）是否立项；③ 项目级收口（转论文/收官——与 Critic 线 V9 amendment 呈报合并决策）。
- 本线 watcher 全部退出（done markers 落盘）；GPU4 释放。

### 批次六十二（2026-09-10 09:15，amendment v2 生效——B3 参照勘误 + 双口径正式数字 + β=0.25 双口径终判）

- **用户拍板**：「执行 amendment v2」（批 61 遗留 ①）。产物 `docs/paper/route2_setflow_b3_amendment_v2.md`（v8_stage1_prep worktree）。
- **勘误（v1 §3.2）**：B3 绝对线 2× base 参照错配（mixed-pool 0.1514 vs generation-internal 0.5131 口径不可比，2× 线 1.146 不可达）。v2 重定：**B 档绝对线 0.55（= base 0.513 × 1.07）/ A 档 0.46（= base 0.410 × 1.12）；0.10 绝对线与 2× 线废除**；Δ门不变（B2 ≥ +0.03 CI 排零）。
...
### 批次六十五（2026-09-10 21:20，D2 calib100 第一级终态——全臂 B2 Δ门 FAIL → 预注册负收口）

- **4/4 β 臂终态**（watcher 自动 20:11-21:07，GPU5/GPU4 轮换；harvest 脚本 str/Path 低级 bug 修复后手动收割，产物 `d2_calib_harvest.json`）。
- **第一级判定（amendment v2 B2 Δ门，calib100，基线 support 0.16 / sc-hit@1 0.573 n=16）**：

| β_r | support | Δsupport [CI] | Δsc-hit@1 [CI] | B2 门 |
|---|---|---|---|---|
| 0.25 | 0.22 | +0.06 [+0.01,+0.11] | +0.082 [−0.048,+0.233] | FAIL（跨零） |
| 0.5 | 0.23 | +0.07 [+0.02,+0.13] | +0.079 [−0.042,+0.230] | FAIL（跨零） |
| 1 | 0.23 | +0.07 [+0.02,+0.13] | +0.100 [−0.018,+0.243] | FAIL（跨零） |
| 2 | 0.22 | +0.06 [+0.01,+0.12] | +0.104 [−0.020,+0.255] | FAIL（跨零） |

- **按预注册终止条件：H-D2 否证，D2 负收口（不晋级 891 确认，不追加）**。
- **边际信号如实入档（非门判据）**：四臂 Δsc-hit@1 点估计 +0.08~+0.10（CI 跨零，n=23 S+ 检验力不足）——**方向与量级略高于 V5 critic 引导**（calib100 同 cohort V5 β sweep 最优 +0.15 假阳性 vs 检索 +0.10 跨零——检索臂的点估计更稳但都不显著）；Δsupport +0.06-0.07 排零正（覆盖增益真实存在，与 V5 β=0.25/0.5 同量级）。结合结构发现（源间 Hamming 42-62%）：**检索信号是「非近邻的弱全局信号」**（可能来自任务级编辑模式先验而非源特异性），其存在不依赖近邻假设——但幅度不足以过门。
- **D2 终局科学结论**：measured 邻域信息双通道（C3 学习 q + D2 直接检索）在当前验证体制下均无法产生显著增益——**与「源间独立性」（结构性发现）和「检验力上限」（n=23 S+）双约束一致**。q 0.5528 + 检索 CI 跨零 + Hamming 42-62% 构成 H4/H5 收口的三重证据。
- 工程注：harvest watcher 的 str/Path bug 系部署疏漏（rc=1 空 verdict），已修复 + 手动收割补链——如实入档。

### 批次六十六（2026-09-10 23:00，用户指示反思推进 → V6.5 探索-利用分离方案落地：891 全量 B2 门离线已过）
...
### 批次六十九（2026-09-11 13:10，V6.5 探索-利用分离终判：B2 Δ门 5/5 臂过 + 3-seed 稳健成立；B2-I 独立口径 FAIL；B3 未过线——诚实全图景）

- **收割修正两处（如实）**：(1) v65_harvest 的 b3_absolute_055 存在运算符优先级 bug（恒 True 假阳性标志）——已修复重算，实际 5 臂 sc-hit@1 0.504-0.545 全部 < 0.55；(2) seed16/17 首发失败（--decoder-seed-base 非原生参数 rc=2）→ 并行会话的 seed-rerun watcher 以预注册种子值（2026091601/2026091701）重跑成功——核实 decoder_seed_base 字段证实有效。
- **B2 Δ门（amendment v2，891 全量，配对 bootstrap）——首个 3-seed 稳健过门的方向**：

| 臂 | sc-hit@1 | Δsc-hit@1 [CI95] | B2 门 | support | recovery |
|---|---|---|---|---|---|
| B256 主 | 0.543 | +0.034 [+0.021,+0.048] | **过** | 0.292 | 0.164 |
| B256 seed16 | 0.545 | +0.040 [+0.027,+0.055] | **过** | 0.302 | 0.169 |
| B256 seed17 | 0.504 | +0.032 [+0.020,+0.047] | **过** | 0.311 | 0.172 |
| B512 | 0.533 | +0.038 [+0.025,+0.053] | **过** | 0.305 | 0.172 |
| B1024 | 0.514 | +0.040 [+0.026,+0.054] | **过** | 0.320 | 0.176 |

- **边际曲线结论**：support/recovery 随 B 单调升（0.292→0.320 / 0.164→0.176）但 sc-hit@1 单调降（0.543→0.514，稀释）——**B256 为排序口径最优探索宽度**；hit@1 unconditional 稳定 0.156-0.165（+27% vs baseline 0.124）。
...
### 批次七十一（2026-09-12 09:35，组合实验 tier-1 终判：D 组合臂三口径全臂最高 + 近可加指纹 → 晋级 tier-2）

- **D 臂 calib100 终态**（修复链完整：第 5/6 处 32 硬编码 [guided per-source 切片 + 预算断言作用域] + 第 7 拦截 [guided evaluate_arm cap 透传——mirror of unguided 补丁]，commits 635f472e/65e06aaa/e2f999eb；首跑 6h 产物完整、断言死后直接收割）。**tier-1 三口径终表**：

| 口径 | B 纯引导 | C 纯探索 | **D 组合** | 模式 |
|---|---|---|---|---|
| Δsc-hit@1 [CI] | +0.019 [0,+0.047] | +0.028 [0,+0.065] | **+0.048 [+0.010,+0.093] 排零** | B+C≈D |
| Δsupport | +0.02 | +0.04 | **+0.07** | B+C≈D |
| ΔOptimus [CI] | +0.029 跨零 | +0.035 跨零 | **+0.050 跨零（四臂最高）** | B+C≈D |
...
### 批次八十五（2026-09-13 03:10，D891_main 速率回升观测 + 周报参照数字冻结产物核实）

- **D891_main 心跳更新**：done 74→111/891，rate 0.0016→0.0018 源/s（ETA 6.0→5.1 天）——GPU5 争用缓解后提速；三臂 CPU jiffies 持续增长（健康推进），seed16/17 效率约为 main 一半（231% vs 208%×双臂共处），ETA 量级一致。
...
### 批次九十二（2026-09-13 06:10，里程碑：三臂全部确认稳定推进——seed16/17 首心跳到达）

- **D891_seed16/17 首心跳**：各 37/891（发射后 ~5.1h，比 main 同期 7.6h 更快——发射时机的 GPU 空闲度更好）；速率各 0.0021 源/s，ETA 各 4.7/4.8 天（~09-17/18 终态）。
- **D891_main 第三次提速**：185/891，0.0026 源/s（ETA 3.2 天 → ~09-16 终态）。
- **三臂终态窗口统一落在 09-16 至 09-18——全部在本周内**（周五 09-18 截止），核心交付时间线确认。
- 健康判据第四次复核通过（心跳 + CPU jiffies + SM 活性三重一致）；批次九十一的"共享卡吞吐减半"推断被证实（0.0021 ≈ main 独占速率的一半）。
- 双 watcher + 定时监控继续值守；终态后链条（收割 → 终判 → D16-C → 周报）全就绪。

### 批次九十三（2026-09-13 07:10，D891_main 第四次提速：0.0029/s，ETA 2.7 天）
...
### 批次七十二（2026-09-18 01:00，组合实验 tier-2 预览（2/3 臂终态）：D ≈ C 模式确立 + Optimus 口径 seed 波动）

- **D891_main/seed16 终态**（wall 117.5h/100.2h——周级争用下跑完；recovery 0.2479/0.2503 = **基线的 2.06/2.08×**）；seed17 在途 851/891（ETA ~5h）。seed16 的 failed.json 系产物清理竞态残留（summary 有效，FileNotFoundError 于旧输出目录）。
- **2×2 矩阵 @891 全量（sc-hit@1 口径，2/3 D 臂）**：

| 臂 | Δsc-hit@1 [CI] | B2 门 | ΔOptimus [CI] | B2-I 门 |
|---|---|---|---|---|
| B 纯引导 | −0.004 跨零 | ✗ | +0.014 [−0.012,+0.040] | ✗（复测：批 62 的 +0.036 排零未复现——口径脚本差异待查，如实登记） |
| C 纯探索 | **+0.034 排零** | **✓** | −0.004 跨零 | ✗ |
| D 主 | +0.027 [+0.014,+0.041] 排零 | ✗（差 0.003） | +0.002 跨零 | ✗ |
| D seed16 | **+0.033 [+0.020,+0.048]** | **✓** | +0.017 [−0.013,+0.048] | ✗（CI 下界 −0.013） |

- **机制终判（预览，待 seed17）**：**D ≈ C 模式确立**——组合臂复制探索增益（support 0.293-0.299 = C 的 0.292 水平；sc 两 seed 0.027/0.033 跨 C 的 0.034），引导成分在 recovery 口径零贡献（同 B 单独无增益）但**未吞噬探索增益**（非破坏性 H-int）；Optimus 口径 D 主 +0.002 / seed16 +0.017（B 的功能增益在叠加后部分保留但 CI 跨零，seed 波动 0.002-0.017 大）。
- **诚实归档**：calib100 的「D 三口径全最高 + 近可加」在 891 尺度**部分外推**——sc/support 口径的 D≈C 成立，但 Optimus 口径的近可加（+0.050）未复现（D 两臂 +0.002/+0.017 vs calib +0.050）；**B 臂的 +0.036 排零 Optimus 增益在本次复测中 CI 跨零**（+0.014 [−0.012,+0.040]）——批 62 双口径与本次脚本的 seed/配对细节差异待复核（列为待查项）。**结论暂定：H-add 在 recovery 口径成立（D=C 且 > B）、在 Optimus 口径不成立（增益不出门）**——最终判定等 seed17。

### 批次一百零四（2026-09-18 10:35，seed17 批次间超慢诊断——健康证据链成立，终态窗口修订）

- **现象**：seed17 心跳停 851/891（last Sep 17 14:55），已 19.6h 无新批次写入；进程 285149 存活。
- **判别诊断（三重证据判定为活，非卡死）**：① CPU 30s 采样 +122s ≈ **406%**（多线程重计算，高于历史 105-264% 样本）；② GPU4 utilization 65% 且 seed17 独占 23.5GB；③ 日志 mtime 停 Sep 13 00:57 为全缓冲 stdout 的预期行为（python print 未 flush，非零输出）。**alive-检测（watcher pgrep）+ SM 活性 + CPU jiffies 三重一致 = 健康推进中，批次内慢源所致**。
- **速率归因**：851→888 批次已 19.6h，超过 seed17 自身历史极值（9.5h）进入 main 臂慢批次带（25h）——GPU4 新增外部小进程（1.1-2.1GB ×3）+ MRL 慢源叠加。
...
### 批次一百零六（2026-09-19 01:30，ERK v2 训练臂全链终判——数据+参数两侧证伪闭合，MRL 源内结构排序判定性证据）

- **执行链**（amendment v1 APPROVED 后零自由裁量）：特征单测 5/5 绿（特征构造=E7-a v2 探针形态逐位锚定）→ 冒烟全链（1 步 val 0.184）→ 主臂 6 epochs（GPU3 整卡，90s）→ gap backtest（镜像 V5 train-backtest 协议：源序 2000 截断 + per-源前 32）→ G4 结构池探针（D15-2 口径 × MRL 任务域适配）→ 四门汇总。
- **模型**：1,177 参数（W=7 blocks×64 ctx=448 + b=729 源残差）——比 V5 170M 少 5 个数量级；零预训练特征；零合成数据；Huber 同 V5 target_scale 0.5232；CUDA 硬门证据齐（cpu_fallback=false）。
- **G1_FAIL（gap 门 0.3815 > 0.30；val 门 0.2015 ≥ 0.135 PASS）**：对照链 rho_train/rho_val/gap：V5 0.928/0.134/0.794 → D16-C 0.956/0.132/0.824 → **ERK 0.583/0.201/0.382**——记忆减半（train −37%）、**MRL VAL 历史新高 0.2015**（V5 0.1354 的 1.49×，s_mprau_in 外首个跨 0.2 的 MRL 行）、gap 减半。
- **G2 PASS（by design）**：ERK 无 polyA 头，V5 polyA 主行 0.8219 冻结未动（amendment §7 语义澄清条款）。
- **G3 not triggered**：gap 0.38 < 0.50 重记忆带 → 参数侧非"独立无效"，而是"半程有效"（记忆路径被显式先验显著压缩但未根除）。
- **G4_FAIL（判定性）**：MRL 结构池（231 源，oracle 结构采样 graded support 100%）graded_sc_hit1 = **0.0**——与 D15-2 polyA（V5 0.0）、D16-C（probe 0.0）逐位一致。**三代模型家族（170M 端到端 / D16-C 数据增强 / 1.2K 显式先验）结构池全零信号。**
- **科学结论（amendment §4 诚实条款兑现）**：①「数据+参数两侧均未使表征学结构」完整现象学成立——D16-C 与 ERK G4 并列：源内 top-1 排序零信号是**数据集几何的固有不变量**（非函数类可修复）；② 但消歧结论：ERK 的 val 0.20 主要由源残差贝叶斯收缩驱动，能量表对源内判别无力 → MRL 当前可学信号 = 源级平均效应；③ **两参数侧假说分开定论**：「容量性记忆偏好」部分成立（显式先验确实压缩了记忆：train 0.928→0.583），「结构可学性」不成立（G4 0.0）——与 H2 失效链（判别力靠数据分布）合流为：**MRL 泛化瓶颈 = within-source 结构密度（4.9 cand/源），函数类两侧干预均无法替代真实数据几何**。
- **下一步裁断（解锁外部数据路线）**：数据侧（合成增强）与参数侧（显式先验）双双到达同一天花板「val ≈ E7 一阶闭式上界 ≈ 0.207」——**要突破 0.2 只能拿真实高密度数据**（全新外部 MRL dense library，如 GSE114002 designed-lib 扩充或新 publicação），合成与函数类路线均宣告关闭（双证伪）。此为 ERK amendment §6 分叉条款的正式落点。
- **commit**：runner / pinbacktest / G4 probe / 单测 / adjudication / amendment 拍板记录——本批全部入 W0，push 待。

### 批次七十三（2026-09-19 15:45，组合机制实验 tier-2 终判：3-seed 完整 + 机制定论）

- **seed17 终态**（wall ~110h，三臂全部周级跑完）。**3-seed sc-hit@1 终表**：main +0.027 [0.014,0.041]（门差 0.003）/ seed16 +0.033 [0.020,0.048] **过门** / seed17 +0.029 [0.015,0.045]（门差 0.001）。**3/3 seed CI 排零显著正、2/3 达 +0.03 字面门**——3-seed 稳健条款（3/3 方向一致）**成立**（全部显著正），字面 +0.03 线 2/3（边缘差 0.001-0.003）。**Optimus 口径 3-seed**：+0.002/+0.017/+0.007 全跨零——B2-I 不出门。
- **机制终判定稿**：
  1. **D ≈ C 确立**（三 seed sc 点估计 0.027-0.033 ≈ C 的 0.034；support 0.293-0.306 ≈ C 的 0.292 并超之；recovery 0.161-0.168 = C 的 0.164 水平）——**探索增益完整保留，引导叠加无破坏**（非 H-int）；
  2. 引导成分在两口径均无净增益（sc：D−C ≈ −0.005~0；Optimus：D − C = +0.006~+0.021 但全跨零）——**β=0.25 弱引导的 calib100 信号（含批 62 Optimus +0.036）在 891 复测中均不稳定**（复测 +0.014 跨零；seed 波动主导）；
...
## 批次一百零八（2026-09-19 · Phase 1 构念效度 + 机理分析线全闭合）

- **Task 1.0.2 oracle Δh-probe 对照臂（决定性实验）**：预注册 delta_validity_oracle_prereg_v1.md 先落（commit a2a2919a）；脚本 run_route2_delta_validity_oracle_probe_v1.py（commit 085843b6；复用 te-family 管线 embed/fit_probe/evaluate_task，零新口径）；4 行出数（CUDA cuda:6 MIG 1g.5gb 留证，cpu_fallback=false）：**MRL 两族 NO_SIGNAL**（UTR-LM/RNA-FM 入档行 0.1107/0.1369 系 probe seed 敏感伪信号——seed 20260816 精确复现入档值，换 seed 即塌缩至 0.049-0.064，3-seed 均值 0.0700/0.0869 ≤ 0.10）=「表征差分 ≠ 性质差分」直接证据；**polyA 两族 HEAD/EQUIVALENT**（0.7436/0.7184 跨 seed 稳健，gain ≤ +0.007）= delta 信号真实存在于表征且已兑现；零行 REPRESENTATION_SIGNAL。**协议发现（如实申报）**：既有 frozen-Δ LM 行实现代码本身即 Δh→Δy 回归（te-family/full-coverage docstring 与代码不一致），线性读出下两监督形式数学等价（port-validation 证实）；已入 protocol note，oracle 臂按代码事实 + 3 seeds 实现并显式声明口径。
- **Task 1.1 一阶分解扩展（机理链 B）**：预注册 first_order_decomposition_prereg_v1.md 先落（commit 8f80d7e6）；E7 一阶上下文能量表闭式拟合复刻至 polyA/MPRAU/TE：**polyA 一阶 0.4555 = V5 的 55.4%**（非一阶信号大头 0.27+，外部通用 LM 已拿到）；**MPRAU 0.0825 = V5 的 80.5% / TE 0.0458 = V5 的 79.1%**（两任务我方行几乎就是一阶信号，「结构增益」表述须克制）；判定 = polyA 部分成立 / MPRAU 不成立 / TE 部分成立（B 不适用）；结构洞见：polyA 平均 8.5 edits/record 是唯一有组合空间任务恰为唯一「一阶远低于外部带」任务，任务异质性与数据几何一致。产物 analysis_first_order_decomposition_v1/（json+图+md）。
- **Task 1.2 失效模式分类表（机理链 C）**：analysis_failure_taxonomy_v1/ 落盘——20 行（失效 14：物理 5/范式 5/几何 2/监督 5 + 对照 6）覆盖 9/9 任务；分类规则先写后分；跨类 10 行主+次标注；边缘 case 三项决定（GSE149487-RNA→C4 主 C1 次、GSE186455→C4、LAMAR→C1+ρ_ε 交叉证据）；ICC 出处口径更正登记（HL 真实源 = e0_e1_e3_b2/mechanism_results_v2.json，非 ceiling_icc_20260907）。
...
## 批次一百一十一（2026-09-20 · Task 2.3 矩阵外部行独立复核 + Task 2.4 delta-density v2 held-out 判定与降级执行）

- **Task 2.3 独立复核（65/65 全 PASS，零偏差）**：复核脚本 `scripts/route_a_v3/run_matrix_recheck_v1.py`——不读任何入档指标值，直接从各族 `predictions.jsonl`（137,350 行）逐格重算 task Spearman（`spearmanr(observed, delta_hat)`，镜像 Task-1 评测器单 task 组口径），与 `matrix_v2_results.json` 入档值比对 |Δ|≤1e-6。结果：**65 格全部 PASS，全矩阵 max |Δ| = 0.0（浮点级精确一致）**——含 STCNet MRL 0.814430506981109 等极端格。产物 `benchmark_v2/leaderboard_matrix_v2/analysis_matrix_recheck_v1/recheck_results.json`（含逐格 recompute/archived/abs_delta/per-cell 明细）。矩阵入档值可信性闭合。

- **Task 2.4 delta-density v2 held-out 预测检验（严格按冻结框架 `delta_density_prereg_framework_v1.md` 执行，判定 FAIL，降级条款触发）**：
  - **协议执行**：冻结拟合 = 25 外部行（`analysis_delta_vs_density_20260915`）OLS log10(density)→ρ（斜率 0.3663/decade，截距 −0.0857；拟合复算 Pearson r = 0.9388 与入档一致）；65 新格一律 held-out 只预测不回灌；密度口径：9 既有任务用 25 行同款 within-source 密度表（GSE149487 两格同口径补算：TRAIN 无 source group、48 对记录、密度 1.0），M1 1.0014 / M6 1.5717 / S1 3.6682（各自 row_manifest 的 cand_per_source_density）。禁挑行、禁改带、机械判定。
  - **判定**：in-band 33/65 = **50.77% < 70% 门 → FAIL**。**降级条款执行**：「密度单因子规律」主张降级为**任务特异性现象**——此后措辞禁用"跨任务普适规律"，仅可表述为"已观测 25 行集合内成立"；唯一升级路径 = 只增行 + 下一判定点重评。
  - **偏离结构（判定后解读，非判定）**：偏离非随机——**21 个低于带格 0 个域内**（polyA 格 5 族全低于带：冻结预测 ρ≈0.68 vs 实测 −0.37~+0.31；MPRAU-skew 5 族 −0.17~−0.23）；**11 个高于带格 6 个域内**（STCNet-MRL **0.8144** 超带 +0.65；M1 四个 MRL 域内族 +0.11~+0.17 托底）。带内 33 格集中于域外低密度 ρ≈0 失效带（GSE200304/HL5/M6 各 5/5）。按族：STCNet 4/13 最低（两极偏离：域内 MRL 大超带 + 域外 polyA 大低于带），其余族 7-8/13。
  - **双因子分组探索性读数（EXPLORATORY 非判定门）**：按"评测任务 endpoint 域是否在模型训练语料内"分组（域映射来自 port ledger + 官方 repo 预处理核实；STCNet 语料重叠为实测：VALIDATION 730 候选序列 **730/730** 在其 MPRA-H 训练语料 GSM3130443 human_utrs+snv 内，其中 667 在 train_val、63 在 test 划分）：域内 3/9=33.3% vs 域外 30/56=53.6%；方向分解 above-band 域内 6/11 vs below-band 域内 0/21——**偏离按监督体制系统化分组**。解读：25 行拟合集 r=0.9388 系密度与域内监督共变（polyA 行恰为 polyA 域内模型、低密行恰为域外通用 LM）；65 新格拉开两因子后单因子失准——**同密度 4.9 下 MRL 域内稠密监督 0.81 vs 域外 −0.02；同密度 126.7 下 polyA 域内 0.68-0.75（旧行）vs 域外 0.15-0.31（新行）**。此为 spec v4 双因子 claim（监督体制 × 数据几何）的 held-out 支持证据，密度单因子降级不动摇双因子主张。
  - **STCNet 域内强跨库弱发现**：域内 MRL 0.8144（历史最强外部 MRL 行）与跨库 M1 仅 0.0672（语料同为 MRL 域但配对口径/库不同）的对照 = 域内监督的收益部分绑定语料级记忆（其评测序列直接在训练语料内）；跨到 M1 新库（GSE232927）即回落到通用族水平。
  - **产物**：`experiments/analysis_delta_vs_density_v2/`（results JSON 含 verdict/downgrade_triggered/逐格 in_band + cells JSON + 散点图 png/pdf：25 旧行灰 + 65 新格五色分族 + 冻结拟合线 ±0.10 带 + summary.md 含解读节与降级执行清单）。脚本 `scripts/route_a_v3/run_delta_vs_density_v2.py`。
  - **HydraRNA 修复回写（Task 2.2 收尾）**：单测 JSON 记录其 env 修复真因——非 stale 的 flash_attn 缺失（2.7.4.post1 已在 env），而是 Triton autotuner 在 MIG 非默认多卡 device index 下失败；修复 = 单设备可见性（CUDA_VISIBLE_DEVICES=<mig> 使 MIG slice 为 cuda:0，纯推理 plumbing，零权重/数值变更）；单测 PASS 回写 family_unit_tests.json（含 note 修正）。5 族单测终态全 PASS（GEMORNA 官方例值、LAMAR strict load+sha256、Insight 官方 CSV 对齐 Spearman 0.96、STCNet strict load）。
  - **纪律终验**：protected TEST reads = 0（全程只读 VALIDATION 产物/manifest/canonical/存档 JSON）；只读既有产物（源数据零写入，产物只落新目录）；判定按冻结框架机械执行、零事后调整（容差 0.10 与门 70% 均未触碰）；GSE114002 canonical 读取仅用于 STCNet 语料重叠探针（只读）。

## 批次一百一十二（2026-09-20 · Task 3.4 M1 真实稠密数据干预验证臂发射：预注册冻结 + 训练臂实现 + GPU 排队）

- **预注册 amendment 先行落盘（commit b2e141cd，发射前冻结）**：`docs/paper/m1_intervention_arm_amendment_v1.md`——本臂角色 = 机理干预验证（R6/方向 A 收编），非 SOTA 冲刺。四门冻结：**G1 方向门** = MRL VALIDATION frozen-delta Spearman vs 3-seed ens 0.3158 基线，单 seed 增益 > +0.01 判方向正（3-seed 后 CI 判显著）；**G2 非破坏门** = polyA VALIDATION 主行（V5 0.8219 口径）回落 ≤ 0.02；**G3 效率门** = 时长/峰值显存报告项；**G4 机理门** = G1 方向正才触发，M1 评测行 delta 排序方向性对照（无硬阈值，与 STCNet 0.8144 域内/跨库 0.0672 的语料绑定现象对照）。解释框架预登记：增益 FAIL 也是有效交付（密度因子独立贡献受限的干预证据）。
- **训练臂实现（数据侧单改动面）**：`scripts/route_a_v3/run_route2_m1_intervention_fullft_v1.py` = Route A full-FT V2 逐字克隆（同 backbone/超参/6ep/FINAL-EPOCH-FIXED/评测口径），唯一增量 = M1 语料并入 TRAIN：mrl_converted_20260909 HepG2 r1 + tcell r1/r2（defined_end）keep=1 行，seed 20260920 确定性下采样 300K/池 323,555；**泄流排除 = m1_mrl_eval_row 全部 4,848 条唯一序列 + 4 条鸽笼 TRAIN 重叠标记**；rl_paper 权威标签；与 280K 库联合 z-score、联合 shuffle。发射前校验：py_compile PASS、路径/池/下采样确定性实测 PASS。与 v8_m1_prefinetune 先例偏差（6 文件 589K vs 本臂 3 文件 300K）已在 amendment 如实登记（多域联合 vs 单任务收编口径差异）。
- **发射状态 = WAITING（如实报告）**：发射窗口 W0 统计 GPU 0-5 无空闲整卡（0: 19.8G/26%、1: 35.5G/56%、2: 37.0G/100%、3: 25.6G/60%、4: 32.8G/15%、5: 34.2G/2% 但 5 个他人进程在卡；GPU 6/7 不在任务允许范围；其中 honghuiyang fill_a100_v3 脚本在持续占卡）。按任务预案走 nohup 排队 watcher：`scripts/route_a_v3/launch_m1_intervention_watcher.sh`（pid 3494364）每 600s 轮询 0-5 整卡空闲（used<2G 且 0 进程）+ 120s 二次确认防抢卡，抢到即 nohup 发射 seed 20260920 训练。心跳 = `experiments/xeditcritic_m1_intervention/seed_20260920/heartbeat.json`（WAITING→TRAINING 每 500 步→EPOCH_DONE→DONE）；日志 = 同目录 `m1_intervention_seed20260920.log` + `launch_watcher.log`。
...
## 批次一百一十四（2026-09-20 · Task 4.3+4.4 骨架 v2 落盘：§8 升格完整正结果章 + D5-D8 四项待决清零 + 全论文证据链严谨性审计）

- **交付物两件**（均落 W0 docs/paper/，v1 保留不删）：
  1. **deltabench_main_paper_skeleton_v2.md（545 行）**：基于 v1（commit 3d0f5388，483 行）定向升级，非全文重写。
  2. **deltabench_rigor_audit_v1.md（158 行）**：对 v2 骨架逐核心 claim 的审稿人攻击面审计（十项必查 A1-A10 + 汇总项 A11），产出 SEALED/GAP 分级与补跑实验清单。
- **v2 四项变更（用户指令逐条落实）**：
  1. **§8 升格**（收缩版案例 → 四小节完整正结果章）：8.1 天花板标尺（ICC=0.90 下 91.3% 完成度、关闭剩余可学空间过半 0.0876/0.1657=52.9%；密度 126.7 + 8.5 edits/record 几何离群声明）；8.2 结果（V5 0.8219 Holm p=0.004 秩口径 + 双口径如实：Spearman Δ+0.0876 CI[0.0630,0.1141] 排零胜 / decision 口径 top-1 0.5007 vs APARENT 0.6011 Δ−0.1004 CI[−0.1426,−0.0576] 显著负——归因其 2.74M within-source 同 assay 语料密度优势；APARENT2 0.6810 对照行；V5 单 seed 声明显式化）；8.3 为什么（一阶闭式 55.4% + 组合信号空间唯一 0.2747 + 多任务隔离因果链：CMS 混合 polyA 崩 −0.2261（option3_cmsaug_adjudication per_task_side_effects 实查原文）/ W0 单任务 +47%（0.1987 vs 0.1354）/ V6-V9 失败史一句话带过）；8.4 应用出口（COMB polyA 口径三行 D 臂 +0.3527/+0.4210/+0.4844（n=20=891 池 polyA 源全量 report-only）+ B+C≈D 可加指纹 0.02974/0.02972 + 委员会 constructive 路由 +0.0362 CI[+0.0090,+0.0632] 排零、dev-routed +0.0045 跨零分层并报）。
  2. **§1 贡献列表重组 C1-C4**：C4 = polyA 正例推到 ceiling 91.3%（D5 落点）；Abstract 第 5 句同步（正例 + 分类表 + 三体检）。
  3. **决策记录 v2 节**：D5 polyA 不拆分（用户 2026-09-20 拍板「单独小论文无意义，合并主论文」——v1 §8.4 小论文留位作废）；D6 M5 不纳入维持（spec §8b 默认值，14 族范围不动）；D7 生成线（COMB polyA 口径+可加性）归 §8.4；D8 委员会路由归 §8.4——**四项待决清零**。
  4. **连锁小改**：§2.4 ICC locator 统一改 label_icc_reference（HL 纠错正式落到骨架层）+ §9.3 V5 单 seed 显式声明 + §10.7 新增（polyA 统计协议/COMB tier-2 协议/委员会 dev-holdout 协议）+ 附 B B4/B7 落点更新 + 附 C 统计（§8 claim 10→22 行、总 76→91 行）。
- **审计结论摘要（十项必查）**：SEALED 4（A6 委员会出处/seed/预注册链、A8 oracle 协议披露、A10 ICC locator 纠错核验通过、A11 v2 新引数字全实查一致）/ **GAP-可补跑 6（其中 P0 一项：A1 V5 polyA 主行单 seed——最大攻击面确认，MRL/MPRAU 均有 3/5-seed ens 而 polyA 行独缺；P1 三项：A4 M1 密度双口径敏感性、A5 一阶 α VALIDATION 选择偏差、A9 新行 15 格 MDE）/ GAP-不可补 1（A7 COMB polyA n=20 = 891 池内全量上限，声明 limitation 不补跑）**。关键实查发现：polyA ICC 0.90 在 label_icc_reference 有值但无 n/计算块（ceiling_icc_20260907 仅覆盖 3 任务）→ P0-2 补复算；委员会两变体（constructive 排零/dev-routed 跨零）产物分层完好，v2 强制并报合规。
- **补跑实验清单（默认 P0-P2，GPU 总需求 ~3-4 卡日，穿插 M1 空隙不抢整卡）**：P0-1 V5 polyA 3-seed（GPU 1 卡×9h×2）/ P0-2 label ICC 9 任务复算块（CPU）/ P1-1 M1/M6/S1 密度双口径敏感性（CPU，先 mini prereg 判定以原口径为准）/ P1-2 一阶 TRAIN-only α（CPU）/ P1-3 新行 15 格 per-cell bootstrap CI+MDE（CPU 纯重算）/ P2-1 五族前向 determinism check（paddle/Triton autotune 重点）。P3 可选项（委员会 bootstrap seed 扰动、oracle 扩 2 族）与不补跑项（COMB polyA 扩样——换池 4-6 卡日与 M1 冲突，limitation 声明优先）见审计文档 §三。
- **数字抽查（执行代理，只读实查）**：v2 全部新引数字逐项对源——CMS polya −0.2261/mrl +0.1809（option3_cmsaug_adjudication.json）、W0 +47%（journal 批次 5 原文）、constructive +0.0362 CI[+0.0090,+0.0632]（option2_constructive_ci.json）、dev-routed +0.0045 CI[0,0.0112]（option2_committee_routing.json）、APARENT2 0.6810（result.json）、B+C 0.02974/D 0.02972（comb_tier2_harvest.json verdict.additivity_fingerprint）、top-1 Δ−0.1004 CI[−0.1426,−0.0576]（task1_alignment_results.json ranking.top_1，747 可排名源组）、Holm 家族 9 任务构成（bottomline_adjudication_v1.md 表）、polyA D 臂三行（comb_tier2_polya_aparent.json，n=20 report_only=true 原文）——全部一致；91.3%/52.9% 为自造指标已加定义句与 ICC 上界语义声明（B7 条款扩展）。
