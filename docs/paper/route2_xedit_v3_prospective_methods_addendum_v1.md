...
**Paragraph role — compute.** Every method is limited to 320 forward-equivalents per source. One forward-equivalent is one executed physical batch through one base-flow, value, or individual critic-ensemble member; a high-level scorer call that is internally split into microbatches is not treated as one forward. Critic inference uses microbatches of four candidates, so scoring the 32-candidate cap reserves (3\times\lceil32/4\rceil=24) terminal critic forwards per source. Network forwards executed by the deterministic replay are added to the primary generation record, while cache hits that execute no network are not fabricated as forwards. Reports retain candidate count, forward counts by component, same-cohort wall time, and peak VRAM, followed by an equal-wall-time sensitivity analysis. The full method is compared with unguided SetFlow, source-anchored first-order guidance, one-step critic-rate guidance, generate-then-rerank, and the strongest frozen matched search baseline.

Wall-time sensitivity uses GPU-synchronized per-source timers on A100 devices. Its scope includes replay where applicable and every score required to select a reported candidate, while excluding post-hoc diagnostic scores that do not alter selection. The common budget is the minimum observed full-cohort wall time across the six methods; every method is then evaluated on the same largest fully completed prefix of the frozen 891-source order. A partially completed source is not counted. Because the historical genetic artifact did not retain per-source wall time, the prospective benchmark permits one timing-only execution with the already frozen checkpoint, search budget, hyperparameters, and decoder seed. This execution cannot reselect or alter the strongest baseline, and its prior terminal performance metrics remain unchanged.
