...
- **参数 band 修正（2026-09-19 拍板时点，如实对齐探针形态）**：Energy 表 = n_blocks×64（依 MRL 实际源长度决定，~1.5K-8K）；per-source Residual 头 = ~14.6K；总量 ~15K-25K。原 1M-12M band 与 E7-a v2 已 PASS 形态冲突（探针才是唯一合法继承形态），按"继承已 PASS 形态"原则修正，不以参数规模作为验收对象。
- frozen mRNABERT encoder **不参与**（ERK 特征 = 纯显式结构查表，零预训练特征依赖）——与探针一致。
...
## 7. 拍板记录（2026-09-19 已批准）

- [x] 主臂范围（MRL P0 能量头训练 + polyA 参照行不动）确认
- [x] 判定门 ERK-G1~G4 确认（G2 语义明确：polyA 参照=V5 官方 0.8219 主行冻结不动，ERK 无 polyA 头，G2 以"主行未改 + 探针 polyA 0.504 不劣"声明式通过——判读规则：ERK 判定器记录 V5 polyA 行未变）
- [x] seed 系列：training seed = 2026091901（主臂）
- [x] 批准状态：**APPROVED**（2026-09-19，用户「直接按这个方案来」）
