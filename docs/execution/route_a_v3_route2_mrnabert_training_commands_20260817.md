...
## 12:03 C3 long-interval health（2026-08-25）

本地12:00:56在合法窗口后执行当前时点的一次单包远端检查；未对中间错过窗口做密集补查。远端12:03:13时
五个登记PID均为`Rl`且仍位于GPU0/1/2/3/5，CUDA占用为2,120–2,190 MiB；该单次快照只证明登记进程alive，
不解释为性能、训练曲线或完成度信号。
...
第九个实际记录窗口本地09:25:09、远端09:27:31确认状态仍未变化：scheduler存活且已运行44,208秒；八个
Critic arm仍为failure、零summary；SetFlow full/single-mode无terminal artifact，两个固定PID继续在GPU1/GPU2
注册CUDA并占用2,912/2,774MiB。没有读取failure payload或active log/curve/metric，也没有停止、修改或重启
...
本地15:05:15/远端15:07:31统一窗口状态未变：Critic source-only继续GPU5 alive+CUDA 14,342MiB，C0 summary
...
本地21:37:24/远端21:39:41窗口状态未变：Critic source-only仍GPU5 alive+CUDA 15,270MiB；原SetFlow两项
仍GPU1/GPU2 alive+CUDA且无terminal。未读payload/curve/metric/TEST/Evaluation；下一检查`>=22:37:24`。
审计：`audits/route_a_v3_route2_xedit_v4_dual_package_health_20260826_213724.json`。

本地2026-08-27 02:42:43/远端02:45:00低频窗口状态未变：Critic source-only仍GPU5 alive+CUDA
15,270MiB；原SetFlow两项仍GPU1/GPU2 alive+CUDA且无terminal。未读payload/curve/metric/TEST/Evaluation；
不补造heartbeat间隔内未执行的中间轮询，下一检查`>=03:42:43`。审计：
`audits/route_a_v3_route2_xedit_v4_dual_package_health_20260827_024243.json`。

本地03:44:11/远端03:46:29窗口状态未变：Critic source-only仍GPU5 alive+CUDA 15,270MiB；原SetFlow
两项仍GPU1/GPU2 alive+CUDA且无terminal。未读payload/curve/metric/TEST/Evaluation；下一检查`>=04:44:11`。
审计：`audits/route_a_v3_route2_xedit_v4_dual_package_health_20260827_034411.json`。

本地04:45:11/远端04:47:28窗口状态未变：Critic source-only仍GPU5 alive+CUDA 15,270MiB；原SetFlow
两项仍GPU1/GPU2 alive+CUDA且无terminal。未读payload/curve/metric/TEST/Evaluation；下一检查`>=05:45:11`。
审计：`audits/route_a_v3_route2_xedit_v4_dual_package_health_20260827_044511.json`。

本地05:45:41/远端05:47:58窗口状态未变：Critic source-only仍GPU5 alive+CUDA 15,270MiB；原SetFlow
两项仍GPU1/GPU2 alive+CUDA且无terminal。未读payload/curve/metric/TEST/Evaluation；下一检查`>=06:45:41`。
审计：`audits/route_a_v3_route2_xedit_v4_dual_package_health_20260827_054541.json`。

本地06:46:42/远端06:48:59窗口：SetFlow single-mode已SUMMARY terminal并自然退出，payload保持0 read；full
...
本地08:47:43/远端08:50:03窗口状态未变：SetFlow single-mode保持SUMMARY terminal且payload 0 read，full
仍GPU1 alive+CUDA 2,912MiB；Critic source-only仍GPU5 alive+CUDA 15,270MiB，既有terminal payload未读。
未读active curve/metric/TEST/Evaluation；下一检查`>=09:47:43`。审计：
`audits/route_a_v3_route2_xedit_v4_dual_package_health_20260827_084743.json`。

本地09:49:12/远端09:51:30窗口状态未变：SetFlow single-mode保持SUMMARY terminal且payload 0 read，full
仍GPU1 alive+CUDA 2,912MiB；Critic source-only仍GPU5 alive+CUDA 15,270MiB，既有terminal payload未读。
