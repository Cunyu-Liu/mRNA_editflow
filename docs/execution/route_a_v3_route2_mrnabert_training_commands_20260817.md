...
同信息最强已完成 baseline 的 task-macro Spearman 为 `0.131714`；Huber 单 seed 的增量为约 `+0.01827`，仍不能代替 candidate-permutation、source-only 或 three-seed 判断。uncertainty audit 起初因 float32 训练张量与 JSON 十进制复算产生约 `2.5×10⁻⁸` 的舍入差而误停，现已用明确的 float32 重算容差修复并保留实质漂移拒绝。Huber 的 candidate-permutation 与 parameter-matched source-only controls 已于 2026-08-19 01:19 在 GPU 0/5 启动。source-only control 已于 07:05 完成：task-macro Spearman 为 `0.025703`、global Spearman 为 `0.068596`、prediction std / target std 为 `0.001301`，明显弱于完整模型的排序信号；但其 standardized MAE 为 `1.841805`，低于完整模型，因此仍需等待 candidate-permutation 后按冻结的联合规则裁决，不能只凭一个指标提前通过。Development TEST、Evaluation outcomes、final seeds、全量 refit、LOSO readiness 和 guided XEditFlow 均未越级打开。中央训练表当前为 91 个唯一尝试、72 列：82 completed、1 running、3 failed、3 incomplete、2 stopped。
...
- parameter-matched source-only control 已完成：task-macro Spearman `0.025703`，比完整 Huber 低 `0.124285`；candidate-permutation 仍在正常运行，只有两项 control 的冻结联合判定通过后才进入三个 final seeds；
