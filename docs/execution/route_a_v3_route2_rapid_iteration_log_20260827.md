...
### 后续建议（对迭代工作的实用启示）
- 本扫描闭环了「多样性 vs 保真度」的推理端决策地图：**零重训**即可按目标取点。
- 按需选择（给定下游权重）：
  - 高保真 / recovery / top-k 优先 → T 偏低 + s=4.0（T=0.5,s=4.0: recovery 0.2854 / top_k 0.2327）。
  - unique / 多样性优先 → T=1.5 + s 降低（T=1.5,s=0.25: unique 0.5888）。
  - 平衡带 → s∈{1.0,2.0}、T∈{0.75,1.25}（recovery≈0.26、unique≈0.57）。
- T 饱和区为负信息：T=1.0 与 T=1.25 逐点一致（p^(1/T) 恒等/饱和），**未来不必扫 T>1.0**，可直接跳轴；
  该重合也可作脚本数值稳定性/实现正确性的间接校验（若未来改动后不再重合需复核）。
- s（stop_rate_scale）是主旋钮、T 是次要调节：同 T 下 s 0.25→4.0 recovery 单调 +~18%、unique 单调 −~7%
  （T=0.5 例: 23.9→28.5 对 54.6→50.7）；同 s 下 T 0.5→1.0 unique 增 ~3%（s=0.25: 54.6→58.4），T>1.0 无增量。
  → 实操：提升独特率优先调 s（强、可预测），T 仅在窄段微调。
