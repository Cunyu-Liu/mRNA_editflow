...
| RL mechanism              | 多步/跨区域 synergy 经 counterfactual 与 ablation 支持       | **PARTIAL**（v3 下调：P1-13 1000-wild-type panel 检测到的"synergy"实际是 **5'UTR-internal multi-edit synergy**，因为 `LocalTranslationOracle` 只消费 5'UTR + CDS 前 12 nt，CDS/3'UTR arm 严格为 0；syn_sum=+0.0043±0.0467, p=0.00338 **不**满足预注册 α=0.001, Cohen's d=0.0927 small effect；cross-region (5'UTR × CDS × 3'UTR) synergy **仍未被检验**，待 P2-01 用 P1-04 multi-region ensemble 替换 oracle 后重跑） |
...
| **Stage A 100k 健康**（新增） | learning curve 显示 held-out 增益、NaN/AMP 可控、correctness test 通过 | **STOPPED**（P1-00 决策完成：`docs/stage_a_100k_health_decision.md`；4 个 run (seed 0/1/2/5) 全部 `stop_no_progress`，loss stable, grad diverging, AMP fallback 0.29–0.44, 无 step-level checkpoint；duplicate-`sample_cond_pt` 经审查不存在（roadmap 描述的是历史版本）；**项目目前无可用 trained backbone**；建议 P2-02 修复 AMP/LR/grad clipping/loss reduction 后重启 1 个 seed + step-level checkpoint，或 pivot 到 frozen foundation backbone） |
...
任务:
  ...
  12. P2-12 Leplek 2022 PERSIST-Seq 集成 + Oracle #3 增强:
      ...
      - 输出 docs/p2_12_leplek_integration.md + Oracle #3 v1.2 artifact
验收:
  - P2-01 cross-region synergy 真伪有定论 (GO/borderline/NO-GO), docs/cross_region_synergy_finding_v2.md 存在且 multi-region oracle 下统计检验完整
  - P2-02 Stage A 修复或 pivot 决策完成, 若有可用 checkpoint 则 10k steps 内 held-out 有增益
  - P2-03 leakage-free headline eval 完成, ≥3 training seeds, family-cluster bootstrap CI, primary endpoint 预注册
  - P2-04 split contract 强制消费, 单元测试覆盖
  - P2-05 RL-2 pilot 完成 (若 P2-02 unblock), ≥3 policy seeds, 等 query 优于 beam/SA 或明确降级
  - P2-06 Innovation 2 full-mRNA validation 完成 (若 P2-01 GO), synergy-trained policy 显著优于 random policy
  - P2-07 CodonGPT REINFORCE reproduction 完成或降级文档 freeze
  - P2-08 外部 SOTA matched-budget: ≥4/6 方法 protocol-aligned measured, 10-seed paired test
  - P2-09 OOD robustness: family/source stress 完成, cell/cargo 标记 P3
  - P2-10 alternative backbone 决策 (若 P2-02 pivot)
  - P2-11 full-length MPRA 设计 freeze + wet-lab 合作启动
  - P2-12 Leplek 2022 集成 + Oracle #3 v1.2 locked
  - 更新 next_steps_sota_roadmap.md Section 10 dashboard: Statistical validity / Internal held-out gain / RL independent gain / RL mechanism / External SOTA fairness / Stage A 100k 健康 六项升级
降级路径:
  - P2-01 NO-GO: 壁垒 2 降级 null finding + 方法学贡献, 主 claim 转壁垒 4+1, 投稿调 Nature Methods/Nat Comm/NAR, Innovation 2 投 NeurIPS/ICML workshop, P2-06 跳过
  - P2-02 失败: pivot P2-10 frozen foundation backbone + MEF head, Stage A 自训练推迟 P3
  - P2-05 不胜 beam/SA: RL 不进入主模型, 保留诚实负结果, 主算法回到蒸馏/搜索
  - P2-07 受阻: 降级 pretrained checkpoint only, 记录 blocker
  - P2-08 protocol fidelity 仍不足: 报告 partial, 不 claim superiority, 只 claim paradigm difference
  - P2-11 wet-lab 未启动: 只交付设计 freeze, 投稿降至计算方法期刊, wet-lab 推迟 P4
不在范围内:
  - P3-07 D3 universal constrained RL (依赖 P2-05/06 验证)
  - P3-08 等 reward-query RL/search SOTA compute-normalized table
  - P3-09 test-time personalization ablation
  - P4 prospective wet-lab 执行本身 (P2-11 只交付设计 freeze + 合作启动)
  - P5 投稿包
  - Innovation 3/4/5 实现 (除非 P2-01 NO-GO 且需补强壁垒 4)
```