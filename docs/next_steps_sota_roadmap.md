...
> 版本：2026-07-19 P0-v2-completion revision（在 2026-07-18 RL-centered revision 基础上更新）
...
> 本次修订摘要：(1) 反映 P0 Data Reconstruction v2 已完成，4 个 split manifest + combined bundle 全部 `paper_eligible=true`；(2) 加入独立批判性评估结论；(3) 加入 Stage A 100k 健康决策作为 P1 紧急前置；(4) 重写下一阶段 goal 以便 trae 自主执行。
...
| 维度                | 判断                                             | 说明                                                         |
| ------------------- | ------------------------------------------------ | ------------------------------------------------------------ |
| 目标是否清晰        | **主方向清晰，壁垒 2/4/1/3 优先级明确**          | 战略定位：重新定义 mRNA 设计问题（minimal-edit + cross-region synergy + RL 算法创新），不追 de novo SOTA；壁垒 2 (cross-region synergy) > 壁垒 4 (RL 算法创新) > 壁垒 1 (regulatory-grade minimal-edit) > 壁垒 3 (independent oracle + 合作湿实验) |
| Todo 是否细致       | **P1 已重新设计，14 个 todo + 12 项验收门槛**    | P1-00 (Stage A 健康) / P1-02A/B (真实 + 自建 full-length MPRA) / P1-04/05 (predictor + oracle) / P1-07/08/09 (RL correctness) / P1-12 (Innovation 1+2) / P1-13 (counterfactual panel) / P1-10/11 (split 强制 + long-view) |
| 能否实现预期 SOTA   | **资源充足设定下，按壁垒 2+4 路径有条件可行**    | 不追 de novo SOTA（GEMORNA/mRNA-GPT 工程规模优势大）；以 cross-region synergy 机制发现 + counterfactual RL 算法创新 + regulatory-grade minimal-edit 建立差异化 SOTA；最大风险是 synergy 不显著（降级路径已规划） |
| 是否足够顶刊        | **目标 Nature Biotechnology 主投 + NeurIPS/ICML 副投** | 核心 claim: cross-region synergy 机制发现 + counterfactual RL 算法创新 + regulatory-grade minimal-edit + 合作湿实验 validation；Innovation 2 (counterfactual synergy RL) 单独投 NeurIPS/ICML |
| 科研故事是否吸引人  | **有强潜力，壁垒 2 是机制性发现**                | "Cross-region synergy via counterfactual RL" 是领域空白；如果 synergy 显著为正，这是可发表的新生物学发现（不只是工程改进）；壁垒 1 (regulatory-grade) 提供 FDA 审评维度差异化 |
| 数据集是否全面      | **Layer A 通过，B/C/D/E/F/G 待建；P1-02B 自建 full-length MPRA 是数据壁垒** | Layer A (GENCODE/RefSeq) v2 audit 通过；B/C (MPRA + stability) P1-02A/P1-03 接入；**P1-02B 自建 full-length joint mRNA MPRA 是领域空白，6-12 个月竞争者复制壁垒** |
| 下游任务是否全面    | **D1-D7 定义良好，P1-13 counterfactual panel 是新增机制发现任务** | D1-D7 + 新增 counterfactual cross-region synergy panel（5 arms × 1000 wild-type）；P1-13 是壁垒 2 的第一个机制性发现 |
| 测评是否全面        | **覆盖广，科学有效性待 P1-04/05 独立 oracle 落地后补齐** | 缺 leakage-free OOD（v2 后家庭级已部分满足）、独立 oracle、训练重复、校准、湿实验；外部 SOTA 4/6 measured 但 protocol fidelity 不足；P1-04/05 完成后独立 oracle blocker 清除 |
| 架构是否最优        | **当前不是，但优先级低于壁垒 2/4**               | region adapter 严格负结果、cascade 无稳定 top-1 提升、aux_struct 零 target、rate field 与 ranker 共享 head；P1 优先做壁垒 2/4，架构升级在 P2-P3 |
| RL 是否值得成为主线 | **是，壁垒 4 是核心算法创新**                    | Innovation 1 CTO + Innovation 2 counterfactual synergy RL 是核心；Innovation 3/4/5 作为 parallel/backup；Innovation 2 可单独投 NeurIPS/ICML |
| Cross-region synergy 是否值得成为 primary claim | **是，壁垒 2 是领域空白** | 没有任何现有方法显式学习或量化 cross-region synergy；counterfactual panel + synergy score + RL 优化 = 机制发现 + 算法创新双壁垒；降级路径：若 synergy 不显著，壁垒 4 + 壁垒 1 仍可支撑投稿 |
...
| Independent oracle        | 真实标签、cross-fit、final oracle 独立                       | **NO**      |
| Statistical validity      | ≥3 training seeds + family-level inference                   | **NO**      |
| Algorithm identity        | greedy 与 true flow 明确，correctness tests 通过             | **NO**      |
| RL identity/correctness   | 完整 action distribution、STOP、trajectory log-prob、tiny exact MDP | **NO**      |
| RL independent gain       | 等 query 强搜索 + ≥3 policy seeds + final oracle transfer    | **NO**      |
| RL mechanism              | 多步/跨区域 synergy 经 counterfactual 与 ablation 支持       | **NO**      |
| Reward-hacking resistance | 三 oracle、KL/uncertainty/OOD guard、adversarial audit       | **NO**      |
| Internal held-out gain    | 强基线、独立 test、实质 effect                               | **UNKNOWN** |
| External SOTA fairness    | ≥3 类方法 protocol-aligned                                   | **NO**（4/6 measured 但 protocol fidelity 不足） |
...
| **Stage A 100k 健康**（新增） | learning curve 显示 held-out 增益、NaN/AMP 可控、correctness test 通过 | **NO**（`manual_review_required`，AMP fallback 0.2-0.35，grad P99 2200-8900） |
