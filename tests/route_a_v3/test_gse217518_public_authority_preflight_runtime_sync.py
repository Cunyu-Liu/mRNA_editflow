...
SCRIPT_PATH = (
    STAGING_ROOT
    / "scripts/route_a_v3/gse217518_public_authority_preflight_runtime_sync.py"
)
SPEC = importlib.util.spec_from_file_location("evt046_runtime_sync", SCRIPT_PATH)
...
def unknown_l_and_i_config() -> dict[str, Any]:
    ...
    ledger.update({"status": runtime_sync.UNKNOWN, "commit": runtime_sync.UNKNOWN})
...
def bind_ledger(config: dict[str, Any]) -> None:
    ledger = config["repository_authority"]["predecessor_ledger"]
    ledger.update({"status": "BOUND", "commit": "0" * 40})
...
def make_context(
    ...
    prepared = allowed_root / "evt046-job"
    run_root.mkdir()
    allowed_root.mkdir()
...
def install_fake_repository_authority(
    ...
    *,
    ...
    implementation_commit = "1" * 40
    binding_commit = "b" * 40
    script_payload = b"bound EVT046 runtime publisher\n"
    test_payload = b"bound EVT046 focused test\n"
...
def test_disk_bound_l_and_unknown_i_freeze_evt045_report_and_runtime_delta() -> None:
    config = unknown_i_disk_config()
    ...
    assert ledger["commit"] == "94b846a383b5300414f0848bf3fac620ef7c2d27"
    assert [item["sha256"] for item in ledger["frozen_blobs"]] == [
        "9e72e82e7b8a941fd2e3066a33c4e3ce7fc733134e62776e273c40b1b118fb9e",
        "b6e8a75f512ea8f865b031b3fde5f8a491481fba38c7d63070c150694b68f440",
        "edb940cbb46e9e54fb8267a29f0b5d48020be933defb96a3b3de89f673f18f98",
        "ee812cbe3aed15ad0d22187e872b8fbfe8449469b5c5818d7e46458cc51c3c04",
    ]
    assert {
    ...
    assert config["runtime"]["predecessor_event_count"] == 45
    assert config["runtime"]["successor_event_count"] == 46
    assert config["runtime"]["predecessor_manifest_output_count"] == 187
    assert config["runtime"]["successor_manifest_output_count"] == 192
...
def test_ledger_authority_unknowns_cannot_be_partially_backfilled() -> None:
    ...
    config["repository_authority"]["predecessor_ledger"]["commit"] = "0" * 40
    config["implementation_binding"]["compiled_core_sha256"] = (
        runtime_sync.compiled_core_sha256(config)
    )
...
def test_disk_unknown_i_stops_before_authority_report_or_runtime_io(
...
def test_normal_transaction_adds_exact1_plus_exact4_and_preserves_truth(
    ...
    assert events[-1]["event_id"] == "A1-EVT-046"
    assert events[-1]["scientific_state_changed"] is True
    assert events[-1]["evidence_surface_changed_since_evt045"] is True
    assert events[-1]["evidence_gate_statuses_changed_since_evt045"] is False
    ...
    assert len(manifest["outputs"]) == 192
    assert manifest["outputs"][:187] == old_manifest["outputs"]
    assert manifest["outputs"][187:188] == config["registered_artifacts"]
...
def test_context_orders_authority_before_evidence_and_runtime(
    ...
    with pytest.raises(runtime_sync.PublicationError, match="ordering proof"):
        runtime_sync.prepare_runtime_sync(
            prepared_directory=(
                Path(config["runtime"]["allowed_prepared_root"]) / "evt046-test"
            ),
            recorded_at="2026-08-12T20:00:00+08:00",
            production=True,
        )
