...
CONFIG_PATH = STAGING_ROOT / "configs/route_a_v3_dec022_authority_runtime_sync_v1.json"
SCRIPT_PATH = STAGING_ROOT / "scripts/route_a_v3/dec022_authority_runtime_sync.py"
SPEC = importlib.util.spec_from_file_location("dec022_authority_runtime_sync", SCRIPT_PATH)
...
def _synthetic_predecessor(
    ...
    prior_authority = {
        ...
        "scope": "HISTORICAL_DEC021_AUTHORITY",
    }
    status = {
    ...
    events.append(
        {
            ...
            "event": (
                ...
                "EVIDENCE_REGISTERED_RUNTIME_SYNCED"
            ),
            "decision_id": "V3-DEC-021",
...
def _make_context(
    ...
    prepared = allowed_root / "evt054-job"
    run_root.mkdir()
    allowed_root.mkdir()
...
def _install_fake_repository(
    ...
    *,
    ...
    binding_commit = "b" * 40
    script_payload = b"DEC022 authority runtime sync producer I\n"
    test_payload = b"DEC022 authority runtime sync focused test I\n"
    binding = config["implementation_binding"]
    binding.update(
        {
            "status": RUNTIME_SYNC.BOUND,
    ...
    for index, item in enumerate(config["repository_authority"]["authority_files"]):
        seed = f"DEC022 authority blob {index}\n".encode()
...
def test_disk_config_is_strict_real_i_or_b_with_exact10_authority_bound() -> None:
    ...
    assert authority["authority_commit"] == (
        "4fa39abca424bb6ff82e43a847332e92934b278b"
    )
    ...
    assert config["runtime"]["predecessor_event_id"] == "A1-EVT-053"
    assert config["runtime"]["successor_event_id"] == "A1-EVT-054"
    assert config["runtime"]["predecessor_manifest_output_count"] == 229
    assert config["runtime"]["successor_manifest_output_count"] == 233
...
def test_synthetic_bound_disk_config_hashes_and_normalises_to_i(
    ...
    script_payload = b"DEC022 runtime implementation exact3 I\n"
    test_payload = b"DEC022 runtime focused test exact3 I\n"
...
def test_repository_a_i_b_lifecycle_is_audited(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = _bound_config()
    ...
    assert audit["status"] == (
        "PASS_EXACT10_A_EXACT3_I_CONFIG_ONLY_B"
    )
    assert audit["implementation_commit"] == "6" * 40
...
def test_repository_i_config_drift_is_rejected(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = _bound_config()
    payload = _install_fake_repository(monkeypatch, config, drift_i_config=True)
    with pytest.raises(RUNTIME_SYNC.RuntimeSyncError, match="invalid JSON"):
        RUNTIME_SYNC.audit_production_repository_authority(config, payload)


def test_successor_is_exact_evt054_and_preserves_current_scientific_truth(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = _bound_config()
    predecessor = _synthetic_predecessor(config, monkeypatch)
    ...
    event = events[-1]
    assert event["event_id"] == "A1-EVT-054"
    ...
    assert status["gse256185_dec022_aggregate_row_level_preflight_status"] == (
        "AUTHORIZED_NOT_RUN"
    )
    assert manifest["registered_artifact_count"] == 5
    assert len(manifest["outputs"]) == 233
    assert manifest["outputs"][:229] == RUNTIME_SYNC.load_json(
    ...
    assert sync["manifest_registered_artifact_count_before"] == 5
    assert sync["manifest_registered_artifact_count_after"] == 5
...
def test_partial_publication_retries_as_exact_prefix_and_validates(
    ...
    assert result["event_id"] == "A1-EVT-054"
    assert result["manifest_output_transition"] == "229_TO_233"
    ...
    assert published["event_id"] == "A1-EVT-054"
    ...
    assert validated == {"status": "PUBLISHED_VERIFIED", "event_id": "A1-EVT-054"}
    ...
    assert events[-1]["event_id"] == "A1-EVT-054"
    assert len(events) == 54
    assert len(manifest["outputs"]) == 233
    assert manifest["registered_artifact_count"] == 5
