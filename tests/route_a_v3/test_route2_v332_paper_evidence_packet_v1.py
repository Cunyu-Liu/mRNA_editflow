...
def test_claim_and_consistency_evidence_references_are_closed() -> None:
    ...
    assert len(evidence_ids) == len(set(evidence_ids)) == 57
...
def test_evidence_source_paths_are_closed_without_overstating_verification() -> None:
    ...
    assert preflight["source_locations_checked"] == len(evidence["sources"]) == 57
    assert (
    ...
    assert preflight["local_or_contract_locations_checked"] == 43
