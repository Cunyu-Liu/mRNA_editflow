#!/usr/bin/env python3
"""COMB tier-2 Optimus calibre: 891 full, D arms (genscore top-32) vs baseline;
also B guide (as-is) for the 2x2 matrix; paired bootstrap."""
...
POOLS = {
    "A_baseline": (X + "/guided_b2_20260903/b2_full_891/unguided/generated_candidates.private.jsonl", False),
    "B_guide": (X + "/beta_full891_20260909/beta_0.25_full/guided/generated_candidates.private.jsonl", False),
    "C_explore": (X + "/pool256_unguided_20260908/b2_full_891_B256/unguided/generated_candidates.private.jsonl", True),
    "D_main": (D + "/D891_main/guided/generated_candidates.private.jsonl", True),
    "D_seed16": (D + "/D891_seed16/guided/generated_candidates.private.jsonl", True),
    "D_seed17": (D + "/D891_seed17/guided/generated_candidates.private.jsonl", True),
}
...
rows = {}
base_scores = None
for tag, (path, sel32) in POOLS.items():
    try:
        gen = load(path)
        ...
        continue
    if sel32:
        gen = {sk: dict(sorted(seqs.items(), key=lambda kv: -kv[1])[:32]) for sk, seqs in gen.items()}
    sc = top1_delta(gen)
    if tag == "A_baseline":
        base_scores = sc
    ...
    rows[tag] = {
        ...
        "d_optimus_ci95": ci,
        "b2i_gate": bool(sum(d) / len(d) >= 0.02 and ci[0] > 0),
    }
out = {
    "schema_version": "route_a_v3_comb_tier2_optimus.v1",
    "gate": "B2-I: d_optimus >= +0.02 CI excl zero",
    "rows": rows,
}
Path(D + "/comb_tier2_optimus.json").write_text(json.dumps(out, indent=1))
