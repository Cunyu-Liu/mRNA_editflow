#!/usr/bin/env python3
"""Fail-closed DEC-019 successor adjudicator for GSE114002.

The adjudicator consumes only immutable, hash-bound aggregate gate records.
It never opens row-level data and never emits sequences or identifiers.  The
checked-in I-state is deliberately implementation-unbound, so production stops
before any evidence path is inspected until a direct config-only B commit binds
the implementation.  Missing future gate descriptors yield an aggregate-only
blocked bundle with zero scientific contribution and zero canonical records.
"""
...
CONFIG_REPO_PATH = "configs/route_a_v3_gse114002_dec019_true_a2_activation_v2.json"
SCRIPT_REPO_PATH = "scripts/route_a_v3/adjudicate_gse114002_dec019_true_a2.py"
TEST_REPO_PATH = "tests/route_a_v3/test_adjudicate_gse114002_dec019_true_a2.py"
...
def config_core_projection(config: Mapping[str, Any]) -> dict[str, Any]:
    """Return the stable scientific/config projection for manifest trust.

    The whole implementation_binding block is excluded.  Therefore the direct
    I -> config-only B transition changes the full config SHA but cannot change
    this projection SHA.  No scientific blocker is derived inside the stored
    core projection from the implementation-binding state.
    """
...
def validate_static_config(config: Mapping[str, Any]) -> None:
    ...
    if HEX64.fullmatch(str(geometry["sha256"])) is None:
        raise AdjudicationError("legacy geometry SHA is not bound")
    _expect_int(geometry["bytes"], label="legacy geometry bytes", minimum=1)
    ...
    _expect_exact(output["output_id"], "ROUTE_A_V3_GSE114002_DEC019_TRUE_A2_ADJUDICATION_BUNDLE_V2", label="output ID")
...
def _evaluate(records: Mapping[str, Mapping[str, Any]], config: Mapping[str, Any]) -> tuple[list[str], int]:
    ...
    source = records["SOURCE_FIELD_AUTHORITY"]["facts"]
...
def _blocked_report(config: Mapping[str, Any], blockers: Sequence[str]) -> dict[str, Any]:
    return {
        "record_type": "ROUTE_A_V3_GSE114002_DEC019_TRUE_A2_ADJUDICATION_REPORT_V2",
        "contract_id": CONTRACT_ID,
        "decision_id": DECISION_ID,
        "protocol_id": PROTOCOL_ID,
        "dataset_id": DATASET_ID,
...
def _success_report(config: Mapping[str, Any], canonical_count: int) -> dict[str, Any]:
    return {
        "record_type": "ROUTE_A_V3_GSE114002_DEC019_TRUE_A2_ADJUDICATION_REPORT_V2",
        "contract_id": CONTRACT_ID,
        "decision_id": DECISION_ID,
...
def _build_bundle(
    ...
    marker = {
        ...
        "record_type": "ROUTE_A_V3_GSE114002_DEC019_TRUE_A2_ADJUDICATION_COMMIT_V2",
        "contract_id": CONTRACT_ID,
        "decision_id": DECISION_ID,
        "protocol_id": PROTOCOL_ID,
        "dataset_id": DATASET_ID,
...
def _publish_bundle(
    ...
    *,
    ...
    try:
        ...
        # Forward-only evidence preservation: never delete or rewrite a partial.
        raise
    inspect_committed_bundle(output)
    return "PUBLISHED"
...
def adjudicate(
    ...
    *,
    ...
    if not _all_evidence_descriptors_bound(config):
        ...
        for slot in config["evidence_contract"]["slots"]:
            payload = _read_verified_evidence(slot, config)
            if slot["slot_id"] == "MECHANICAL_ENDPOINT_GEOMETRY":
                records[slot["slot_id"]] = _validate_legacy_geometry(payload, config)
            else:
                records[slot["slot_id"]] = _validate_common_gate_record(payload, slot, config)
        blockers, canonical_count = _evaluate(records, config)
        report = _blocked_report(config, blockers) if blockers else _success_report(config, canonical_count)
