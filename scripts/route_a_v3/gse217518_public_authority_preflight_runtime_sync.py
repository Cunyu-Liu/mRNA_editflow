#!/usr/bin/env python3
"""Prepare, validate, and publish the A1-EVT-046 runtime sync.

EVT046 registers the existing GSE217518 public-authority preflight aggregate. A
...
The staging candidate leaves the L commit/four blob hashes and the four
implementation-binding scalars UNKNOWN_NOT_ASSERTED. Every entry point fails
closed until L is frozen, I is its direct exact3 child, and config-only B is the
direct child of I. Production then proves L -> I -> B, clean
...
SCRIPT_REPO_PATH = "scripts/route_a_v3/gse217518_public_authority_preflight_runtime_sync.py"
TEST_REPO_PATH = "tests/route_a_v3/test_gse217518_public_authority_preflight_runtime_sync.py"
CONFIG_REPO_PATH = "configs/route_a_v3_gse217518_public_authority_preflight_runtime_sync_v1.json"
...
REGISTERED_ARTIFACTS = [
    {
        ...
        "name": "GSE217518_PUBLIC_AUTHORITY_PREFLIGHT_V1.json",
        "artifact_type": "PUBLIC_AUTHORITY_PREFLIGHT_AGGREGATE_ONLY",
        "absolute_path": (
...
class RuntimeSyncError(RuntimeError):
    """Base class for an EVT046 validation/publication error."""
...
class PredecessorError(RuntimeSyncError):
    """The runtime is not the exact EVT045 predecessor or a supported prefix."""
...
def _validate_runtime_shape(config: dict[str, Any]) -> None:
    ...
    _expect(runtime["predecessor_event_id"], "A1-EVT-045", label="predecessor event id")
    _expect(runtime["successor_event_id"], "A1-EVT-046", label="successor event id")
    for key, expected in (
        ("predecessor_event_count", 45),
        ("successor_event_count", 46),
        ("predecessor_manifest_output_count", 187),
        ("successor_manifest_output_count", 192),
    ...
    _expect(tail["event_id"], "A1-EVT-045", label="predecessor tail event")
    ...
    _expect(
        runtime["sync_name"],
        "A1_GSE217518_PUBLIC_AUTHORITY_PREFLIGHT_RUNTIME_SYNC_V1.json",
        label="sync name",
    )
    immutable_expected = [
...
def validate_static_config(config: dict[str, Any]) -> None:
    ...
    _expect(
        ...
        {
            "schema_version": "route_a_v3_gse217518_public_authority_preflight_runtime_sync.v1",
            "protocol_id": "ROUTE_A_V3_GSE217518_PUBLIC_AUTHORITY_PREFLIGHT_RUNTIME_SYNC_V1",
            "contract_id": "mrna_xeditflow_route_a_v3",
            "phase_id": "A1",
            "dataset_ids": ["GSE217518"],
            "decision_id": "V3-DEC-019",
            "event_id": "A1-EVT-046",
            "event_name": (
                "GSE217518_PUBLIC_AUTHORITY_PREFLIGHT_REGISTERED_"
    ...
    _expect(
        ...
        {
            "integration_id": "GSE217518_PUBLIC_AUTHORITY_PREFLIGHT_V1",
            "manifest_status": (
                "A1_GSE217518_PUBLIC_AUTHORITY_PREFLIGHT_"
                "LEDGER_REGISTERED_PENDING_EVT046"
            ),
            "registered_lineage_ids": LEDGER_LINEAGE_IDS,
            "exact_changed_paths": LEDGER_PATHS,
        },
        label="predecessor ledger static authority",
    )
...
def _validate_recorded_at(recorded_at: str, predecessor_at: Any) -> None:
    ...
    if current.tzinfo is None or predecessor.tzinfo is None or current <= predecessor:
        raise PredecessorError("EVT046 timestamp must follow EVT045 with an explicit offset")
...
def validate_predecessor(
    ...
    """Validate EVT045 before creating any prepared member."""
    ...
    if len(events) != runtime["predecessor_event_count"]:
        raise PredecessorError("predecessor event count is not 45")
    tail = events[-1] if events else {}
    _expect(tail.get("event_id"), "A1-EVT-045", label="predecessor tail event")
    ...
    if not isinstance(outputs, list) or len(outputs) != runtime["predecessor_manifest_output_count"]:
        raise PredecessorError("predecessor manifest output count is not 187")
...
def _event_document(
    ...
    *,
    ...
    return {
        ...
        "decision_id": "V3-DEC-019",
        "predecessor_event_id": "A1-EVT-045",
        "registered_artifact_count": 1,
        ...
        "output_delta_count": 5,
        "manifest_output_count_before": 187,
        "manifest_output_count_after": 192,
        "new_runtime_output_count": 5,
        "scientific_state_changed": True,
        "evidence_surface_changed_since_evt045": True,
        "evidence_gate_statuses_changed_since_evt045": False,
        ...
        "next_phase_authorized": False,
        "detail": (
            "Registered the exact GSE217518 public-authority preflight aggregate in place, "
            "added three immutable EVT045 snapshots and one sync record. The evidence "
            ...
            "before ordinary-public row-level production and unqualified; outer A1 "
            "qualification and "
...
def _build_sync_record(
    ...
    return json_bytes(
        {
            "record_type": "ROUTE_A_V3_A1_GSE217518_PUBLIC_AUTHORITY_PREFLIGHT_RUNTIME_SYNC",
            "event_id": "A1-EVT-046",
            "decision_id": "V3-DEC-019",
...
def _immutable_outputs(
    ...
    # EVT045 already has 187 outputs. EVT046 appends the exact1 GSE217518 report,
    # then three snapshots and one sync record, for 192.
...
def validate_successors(
    ...
    """Check exact3 mutable prefix, exact1+4 output closure, and EVT046 last."""
    ...
    if len(events) != runtime["successor_event_count"] or events[:-1] != old_events:
        raise RuntimeSyncError("EVT046 is not one append-only event")
    event = events[-1]
    _expect(event.get("event_id"), "A1-EVT-046", label="successor event")
    ...
    _expect(
        event.get("evidence_surface_changed_since_evt045"),
        True,
        label="event evidence surface change",
    )
    _expect(
        event.get("evidence_gate_statuses_changed_since_evt045"),
        False,
    ...
    if not isinstance(event.get("at"), str):
        raise RuntimeSyncError("EVT046 timestamp is absent")
    outputs = manifest.get("outputs")
    if not isinstance(outputs, list) or len(outputs) != runtime["successor_manifest_output_count"]:
        raise RuntimeSyncError("successor manifest output count is not 192")
    ...
    if len({item.get("absolute_path") for item in outputs}) != 192:
    ...
    _expect(sync.get("event_id"), "A1-EVT-046", label="sync event id")
    ...
    _expect(
        sync.get("evidence_surface_changed_since_evt045"),
        True,
        label="sync evidence surface change",
    )
    _expect(
        sync.get("evidence_gate_statuses_changed_since_evt045"),
        False,
...
def _published_state_is_exact(config: dict[str, Any], payloads: Mapping[str, bytes]) -> bool:
    ...
    outputs = manifest.get("outputs")
    return (
        events[-1].get("event_id") == "A1-EVT-046"
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if _published_state_is_exact(config, predecessor):
            return {"status": "ALREADY_PUBLISHED_VERIFIED", "event_id": "A1-EVT-046"}
    ...
    return {
        ...
        "manifest_output_transition": "187_TO_192",
        "new_runtime_output_count": 5,
    }
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        try:
            ...
            raise PublicationError(
                "EVT046 was not committed; retry with the same prepared directory"
            ) from exc
        final = _read_runtime(run_root)
        if final != successor:
            raise PublicationError("EVT046 publication finished with non-exact mutables")
        return {
            "status": "PUBLISHED_VERIFIED",
            "event_id": "A1-EVT-046",
            "reused": False,
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if current != successor:
            raise PublicationError("runtime does not exactly match prepared EVT046 successor")
        for name in config["runtime"]["immutable_publish_order"]:
    ...
    return {"status": "PUBLISHED_VERIFIED", "event_id": "A1-EVT-046"}
