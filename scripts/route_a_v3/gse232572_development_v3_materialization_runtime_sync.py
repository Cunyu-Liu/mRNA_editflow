#!/usr/bin/env python3
"""Prepare, validate, and publish the A1-EVT-048 runtime sync.

EVT048 registers two existing public GSE232572 development-v3 materialization
reports in ledger order: the preserved fail-closed attempt first and the
current successful aggregate report second. A production prepare verifies each
report's exact bytes and SHA-256 identity without parsing either JSON body.
...
EVENT_LOG as the commit point. Frozen I1 is the direct exact3 child of the fully
...
SCRIPT_REPO_PATH = "scripts/route_a_v3/gse232572_development_v3_materialization_runtime_sync.py"
TEST_REPO_PATH = "tests/route_a_v3/test_gse232572_development_v3_materialization_runtime_sync.py"
CONFIG_REPO_PATH = "configs/route_a_v3_gse232572_development_v3_materialization_runtime_sync_v1.json"
FROZEN_I1_COMMIT = "79317b4ae756dd273a3c1a64d2778565f8523e0a"
FROZEN_I1_BLOB_SHA256 = {
    CONFIG_REPO_PATH: "b82d02fe99f487709395ea281e963a3a06ee2a5cbf0784b3b057bf07a3480f07",
    SCRIPT_REPO_PATH: "df1b7a9297719dcf70a8e41ef94a8263091a858195ee7ab20744c79526bbbc63",
    TEST_REPO_PATH: "4120eadaff8981db118190e3a3933be33079f9247b2f3c045f751fb2f83cb710",
}
...
REGISTERED_ARTIFACTS = [
    {
        ...
        "name": "GSE232572_DEVELOPMENT_V3_MATERIALIZATION_REPORT.json",
        "artifact_type": (
...
class RuntimeSyncError(RuntimeError):
    """Base class for an EVT048 validation/publication error."""
...
class PredecessorError(RuntimeSyncError):
    """The runtime is not the exact EVT047 predecessor or a supported prefix."""
...
def _ledger_values_are_unknown(ledger: Mapping[str, Any]) -> bool:
    ...
    return len(values) == 10 and all(value == UNKNOWN for value in values)


def _validate_ledger_binding(ledger: Mapping[str, Any]) -> None:
    """Allow only the grouped exact2 UNKNOWN candidate or one fully bound L."""
...
def _validate_runtime_shape(config: dict[str, Any]) -> None:
    ...
    _expect(runtime["predecessor_event_id"], "A1-EVT-047", label="predecessor event id")
    _expect(runtime["successor_event_id"], "A1-EVT-048", label="successor event id")
    for key, expected in (
        ("predecessor_event_count", 47),
        ("successor_event_count", 48),
        ("predecessor_manifest_output_count", 197),
        ("successor_manifest_output_count", 203),
    ...
    _expect(tail["event_id"], "A1-EVT-047", label="predecessor tail event")
    ...
    _expect(
        runtime["sync_name"],
        "A1_GSE232572_DEVELOPMENT_V3_MATERIALIZATION_RUNTIME_SYNC_V1.json",
        label="sync name",
    )
    immutable_expected = [
...
def validate_static_config(config: dict[str, Any]) -> None:
    ...
    _expect(
        ...
        {
            "schema_version": (
                "route_a_v3_gse232572_development_v3_"
                "materialization_runtime_sync.v1"
            ),
            "protocol_id": (
                "ROUTE_A_V3_GSE232572_DEVELOPMENT_V3_"
                "MATERIALIZATION_RUNTIME_SYNC_V1"
            ),
    ...
    _expect(
        ...
        {
            "lineage_id": LEDGER_LINEAGE_IDS[0],
            "artifact_type": REGISTERED_ARTIFACTS[0]["artifact_type"],
            ...
            "superseded_for_current_execution_by_lineage_id": LEDGER_LINEAGE_IDS[1],
    ...
    _expect(
        ...
        {
            "lineage_id": LEDGER_LINEAGE_IDS[1],
            "artifact_type": REGISTERED_ARTIFACTS[1]["artifact_type"],
...
def _validate_recorded_at(recorded_at: str, predecessor_at: Any) -> None:
    ...
    if current.tzinfo is None or predecessor.tzinfo is None or current <= predecessor:
        raise PredecessorError("EVT048 timestamp must follow EVT047 with an explicit offset")
...
def validate_predecessor(
    ...
    """Validate EVT047 before creating any prepared member."""
    ...
    if len(events) != runtime["predecessor_event_count"]:
        raise PredecessorError("predecessor event count is not 47")
    tail = events[-1] if events else {}
    _expect(tail.get("event_id"), "A1-EVT-047", label="predecessor tail event")
    ...
    if not isinstance(outputs, list) or len(outputs) != runtime["predecessor_manifest_output_count"]:
        raise PredecessorError("predecessor manifest output count is not 197")
...
def _event_document(
    ...
    *,
    ...
    return {
        ...
        "decision_id": "V3-DEC-019",
        "predecessor_event_id": "A1-EVT-047",
        "registered_artifact_count": 2,
        ...
        "output_delta_count": 6,
        "manifest_output_count_before": 197,
        "manifest_output_count_after": 203,
        "new_runtime_output_count": 6,
        "scientific_state_changed": True,
        "evidence_surface_changed_since_evt047": True,
        "evidence_gate_statuses_changed_since_evt047": False,
        ...
        "next_phase_authorized": False,
        "detail": (
            "Registered the preserved GSE232572 development-v3 fail-closed report "
            "followed by the current aggregate materialization report, both by exact "
            "bytes in place, then added three immutable EVT047 snapshots and one sync "
            "record. The evidence surface and scientific state changed, but gate "
            "statuses and overall qualification did not. The historical failure remains "
            "preserved and is not the current state. The current 8,068 records are "
            "schema-valid DEVELOPMENT_ONLY, with zero canonical records and zero "
            "ordinary/A1/true-A2 study credit. No private JSONL was registered, read, "
            "copied, or listed. Qualification, redistribution, training, model "
            "selection, next phase, and the scientific claim remain locked."
        ),
    }
...
def _build_sync_record(
    ...
    return json_bytes(
        {
            "record_type": "ROUTE_A_V3_A1_GSE232572_DEVELOPMENT_V3_MATERIALIZATION_RUNTIME_SYNC",
            "event_id": "A1-EVT-048",
            "decision_id": "V3-DEC-019",
...
def _immutable_outputs(
    ...
    # EVT047 has 197 outputs. EVT048 appends two ledger-ordered public reports,
    # then three snapshots and one sync record, for 203.
...
def validate_successors(
    ...
    """Check exact3 mutable prefix, exact2+4 output closure, and EVT048 last."""
    ...
    if len(events) != runtime["successor_event_count"] or events[:-1] != old_events:
        raise RuntimeSyncError("EVT048 is not one append-only event")
    event = events[-1]
    _expect(event.get("event_id"), "A1-EVT-048", label="successor event")
    ...
    _expect(
        event.get("evidence_surface_changed_since_evt047"),
        True,
        label="event evidence surface change",
    )
    _expect(
        event.get("evidence_gate_statuses_changed_since_evt047"),
        False,
    ...
    if not isinstance(event.get("at"), str):
        raise RuntimeSyncError("EVT048 timestamp is absent")
    outputs = manifest.get("outputs")
    if not isinstance(outputs, list) or len(outputs) != runtime["successor_manifest_output_count"]:
        raise RuntimeSyncError("successor manifest output count is not 203")
    ...
    if len({item.get("absolute_path") for item in outputs}) != 203:
    ...
    _expect(sync.get("event_id"), "A1-EVT-048", label="sync event id")
    ...
    _expect(
        sync.get("evidence_surface_changed_since_evt047"),
        True,
        label="sync evidence surface change",
    )
    _expect(
        sync.get("evidence_gate_statuses_changed_since_evt047"),
        False,
...
def _published_state_is_exact(config: dict[str, Any], payloads: Mapping[str, bytes]) -> bool:
    ...
    outputs = manifest.get("outputs")
    return (
        events[-1].get("event_id") == "A1-EVT-048"
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if _published_state_is_exact(config, predecessor):
            return {"status": "ALREADY_PUBLISHED_VERIFIED", "event_id": "A1-EVT-048"}
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        try:
            ...
            raise PublicationError(
                "EVT048 was not committed; retry with the same prepared directory"
            ) from exc
        final = _read_runtime(run_root)
        if final != successor:
            raise PublicationError("EVT048 publication finished with non-exact mutables")
        return {
            "status": "PUBLISHED_VERIFIED",
            "event_id": "A1-EVT-048",
            "reused": False,
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if current != successor:
            raise PublicationError("runtime does not exactly match prepared EVT048 successor")
        for name in config["runtime"]["immutable_publish_order"]:
    ...
    return {"status": "PUBLISHED_VERIFIED", "event_id": "A1-EVT-048"}
