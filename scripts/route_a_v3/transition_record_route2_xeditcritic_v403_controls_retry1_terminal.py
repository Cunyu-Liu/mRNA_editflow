#!/usr/bin/env python3
"""Freeze the failed retry1 Critic-controls package before retry2."""
...
OLD_HEAD = "697043fdbfb904dc98adc74095a1bcaa8d62b0f3"
...
RECEIPT_SCHEMA = (
    ...
    "control_recovery_retry1_terminal_receipt.v1"
)
RECEIPT_STATUS = (
    "XEDITCRITIC_V403_CONTROL_RECOVERY_RETRY1_TERMINAL_RECORDED"
)
...
OLD_RUNTIME_ROOT = (
    ...
    f"v403_control_recovery_retry1_runner_{OLD_HEAD}"
)
OLD_RUNTIME = OLD_RUNTIME_ROOT / "runtime.json"
OLD_OUTPUT_ROOT = (
    ROOT
    / "experiments/xeditcritic_v4/"
    f"screen_seed_20260907_v403_control_recovery_retry1_{OLD_HEAD}"
)
OLD_LOG_ROOT = (
    ROOT
    / "logs/xeditcritic_v4/"
    f"v403_control_recovery_retry1_{OLD_HEAD}"
)
CROSS_ROOT_GATE = (
    ROOT
    / "experiments/xeditcritic_v4/"
    f"screen_seed_20260907_v403_cross_root_controls_retry1_{OLD_HEAD}/"
    "screen_gate.json"
)
CANONICAL_RECEIPT = (
    ROOT
    / "audits/xeditcritic_v4/"
    f"v403_control_recovery_retry1_runner_{OLD_HEAD}_terminal.json"
)
...
class CriticControlsRetry1TerminalError(RuntimeError):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise CriticControlsRetry1TerminalError(message)
...
def write_new_atomic(path: Path, payload: Mapping[str, Any]) -> None:
    require(not path.exists(), f"retry1 terminal receipt already exists: {path}")
    partial = _partial(path)
    require(
        not partial.exists(), f"partial retry1 receipt already exists: {partial}"
    )
...
def _runtime_identity(runtime: Mapping[str, Any]) -> None:
    require(
        ...
        "retry1 controls runtime schema changed",
    )
    require(
        ...
        "retry1 controls runtime lineage changed",
    )
    require(
        Path(str(runtime.get("training_worktree", ""))) == LICENSED_WORKTREE,
        "retry1 controls training worktree changed",
    )
    require(
        runtime.get("ordered_control_run_ids") == list(CONTROL_RUN_IDS),
        "retry1 controls run order changed",
    )
    require(
        runtime.get("cross_root_adjudication_run") is False,
        "retry1 controls runtime reports cross-root adjudication",
    )
    require(
        ...
        "retry1 controls runtime changed historical producer isolation",
    )
    require(
        runtime.get("free_memory_gate_applied") is False,
        "retry1 controls runtime used a free-memory gate",
    )
    require(
        ...
        == 0,
        "retry1 controls runtime reports an unauthorized terminal payload read",
    )
    _protected_reads_zero(runtime, "retry1 controls runtime")
...
def _inspect_jobs(runtime: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    require(isinstance(jobs, Mapping), "retry1 runtime job table is absent")
    require(
        set(jobs) == set(CONTROL_RUN_IDS),
        "retry1 runtime job inventory changed",
    )
    ...
    for run_id, expected_gpu in zip(
        ...
        require(
            ...
            f"retry1 job row is not an object: {run_id}",
        )
...
def _inspect_first_failure(
    ...
    require(
        ...
        "retry1 runtime lacks one first terminal failure",
    )
    for key, expected in EXPECTED_FIRST_FAILURE.items():
        require(
            first.get(key) == expected,
            f"retry1 first terminal failure changed: {key}",
        )
    ...
    require(
        ...
        "retry1 first terminal failure is not stably bound",
    )
    return {
        "run_id": run_id,
...
def run(
    *,
    ...
    require(
        ...
        "retry1 runtime path is not canonical",
    )
    require(
        receipt_path == CANONICAL_RECEIPT,
        "retry1 terminal receipt path is not canonical",
    )
    require(
        not receipt_path.exists(),
        f"retry1 terminal receipt already exists: {receipt_path}",
    )
    require(
        not _partial(receipt_path).exists(),
        f"partial retry1 receipt already exists: {_partial(receipt_path)}",
    )
    ...
    require(
        ...
        "retry1 runtime is not the exact technical terminal",
    )
    require(
        not _partial(runtime_path).exists(),
        "terminal retry1 runtime has a partial sibling",
    )
    ...
    require(
        ...
        and scheduler_pid > 0,
        "retry1 scheduler PID is absent",
    )
    require(
        not process_is_alive(scheduler_pid),
        "retry1 scheduler process is still alive",
    )
    ...
    require(
        ...
        + len(not_run_run_ids)
        == len(CONTROL_RUN_IDS),
        "retry1 terminal job partition is incomplete",
    )
    receipt = {
