...
CONFIG_TEMPLATE="${REPO_ROOT}/configs/route_a_v3_route2_base_flow_g0_position_progress_validation_batched_v3.json"
RUNTIME_CONFIG_ROOT="${ROUTE2_ROOT}/runs/base_flow_g0/runtime_configs"
VALIDATION_OUTPUT="${ROUTE2_ROOT}/runs/base_flow_g0/position_progress_validation_batched_v3"
...
if [[ -e "${VALIDATION_OUTPUT}" ]]; then
  printf 'batched validation output already exists: %s\n' "${VALIDATION_OUTPUT}" >&2
  exit 1
fi

cd "${REPO_ROOT}"
runtime_config="${RUNTIME_CONFIG_ROOT}/base_flow_g0_position_progress_validation_batched_gpu${selected_gpu}_v3.json"
mkdir -p "${RUNTIME_CONFIG_ROOT}"
if [[ -e "${runtime_config}" ]]; then
  printf 'batched validation runtime config already exists: %s\n' "${runtime_config}" >&2
  exit 1
fi
