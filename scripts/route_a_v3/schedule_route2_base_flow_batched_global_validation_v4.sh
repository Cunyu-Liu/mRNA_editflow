...
CONFIG_TEMPLATE="${REPO_ROOT}/configs/route_a_v3_route2_base_flow_g0_position_progress_validation_batched_global_v4.json"
RUNTIME_CONFIG_ROOT="${ROUTE2_ROOT}/runs/base_flow_g0/runtime_configs"
VALIDATION_OUTPUT="${ROUTE2_ROOT}/runs/base_flow_g0/position_progress_validation_batched_global_v4"
...
while [[ -z "${selected_gpu}" ]]; do
  ...
  for gpu in "${candidate_gpus[@]}"; do
    ...
    if [[ "${free_mb}" -ge "${MINIMUM_FREE_MB}" && "${free_mb}" -gt "${best_free_mb}" ]]; then
      selected_gpu="${gpu}"
...
if [[ -e "${VALIDATION_OUTPUT}" ]]; then
  printf 'global batched validation output already exists: %s\n' "${VALIDATION_OUTPUT}" >&2
  exit 1
fi

cd "${REPO_ROOT}"
runtime_config="${RUNTIME_CONFIG_ROOT}/base_flow_g0_position_progress_validation_batched_global_gpu${selected_gpu}_v4.json"
mkdir -p "${RUNTIME_CONFIG_ROOT}"
if [[ -e "${runtime_config}" ]]; then
  printf 'global batched validation runtime config already exists: %s\n' "${runtime_config}" >&2
  exit 1
fi
...
printf '%s starting_global_batched_base_flow_validation gpu=%s\n' "$(date -Is)" "${selected_gpu}"
"${PYTHON}" -u scripts/route_a_v3/run_route2_base_flow_g0_validation_v1.py --config "${runtime_config}"
printf '%s global_batched_base_flow_validation_finished\n' "$(date -Is)"
