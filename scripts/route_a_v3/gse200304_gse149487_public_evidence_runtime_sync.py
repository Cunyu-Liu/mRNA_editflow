#!/usr/bin/env python3
"""Prepare, validate, and publish the A1-EVT-045 runtime sync.

EVT045 registers two existing public-evidence records in the ledger order.  A
production prepare verifies their exact bytes and SHA-256 identities without
parsing either JSON body.  The only other immutable outputs are the three
predecessor snapshots and one sync record.  Mutable runtime files commit in the
order STATUS, RUN_MANIFEST, EVENT_LOG, with EVENT_LOG as the commit point.

The checked-in exact3 I config leaves four implementation-binding scalars
UNKNOWN_NOT_ASSERTED.  Every production operation requires the direct
config-only B descendant and proves the exact L -> I -> B repository lineage,
clean HEAD/upstream/origin equality, and frozen blobs before evidence, prepared
output, or runtime I/O.
"""
...
SCRIPT_REPO_PATH = "scripts/route_a_v3/gse200304_gse149487_public_evidence_runtime_sync.py"
TEST_REPO_PATH = "tests/route_a_v3/test_gse200304_gse149487_public_evidence_runtime_sync.py"
CONFIG_REPO_PATH = "configs/route_a_v3_gse200304_gse149487_public_evidence_runtime_sync_v1.json"
...
LEDGER_COMMIT = "56e0a39c77cd828eff52cd48cb36ffc880737de0"
LEDGER_LINEAGE_IDS = [
...
LEDGER_BLOBS = [
    {
        ...
        "sha256": "539413c13d2da6e10a4c065b3e4bb06d472095bcab63a50b4636d1e4633992f3",
    },
    {
        "path": "docs/execution/route_a_v3_registry_manifest.json",
        "sha256": "ad0370edc0bef7baff4127a75170fc93199959d22632c822076f5af53bdf01d3",
    },
    {
        "path": "scripts/route_a_v3/validate_a0_bundle.py",
        "sha256": "b99c5c1107ab6e17e2291d64f045265713fb5cbb6d77b8bd7ffff1b9ccde4124",
    },
    {
        "path": "tests/route_a_v3/test_a0_integrity_guards.py",
        "sha256": "86a7e4edbcb844df59465612178eaeb29a6c439f505098fbae47fabfbbd86856",
    },
]
REGISTERED_ARTIFACTS = [
    {
        "lineage_id": LEDGER_LINEAGE_IDS[0],
        "name": "GSE200304_DEC019_CHECKPOINT_EXPOSURE_FAIL_CURRENT_PROTOCOL.json",
        "artifact_type": (
    ...
    {
        "lineage_id": LEDGER_LINEAGE_IDS[1],
        "name": "GSE149487_PUBLIC_ASSET_ACQUISITION_V1.json",
        "artifact_type": "GSE149487_PUBLIC_ASSET_ACQUISITION_V1",
        "absolute_path": (
...
class RuntimeSyncError(RuntimeError):
    """Base class for an EVT045 validation/publication error."""
...
class PredecessorError(RuntimeSyncError):
    """The runtime is not the exact EVT044 predecessor or a supported prefix."""
...
def _validate_runtime_shape(config: dict[str, Any]) -> None:
    ...
    _expect(runtime["predecessor_event_id"], "A1-EVT-044", label="predecessor event id")
    _expect(runtime["successor_event_id"], "A1-EVT-045", label="successor event id")
    for key, expected in (
        ("predecessor_event_count", 44),
        ("successor_event_count", 45),
        ("predecessor_manifest_output_count", 181),
        ("successor_manifest_output_count", 187),
    ...
    _expect(tail["event_id"], "A1-EVT-044", label="predecessor tail event")
    ...
    _expect(
        runtime["sync_name"],
        "A1_GSE200304_GSE149487_PUBLIC_EVIDENCE_RUNTIME_SYNC_V1.json",
        label="sync name",
    )
    immutable_expected = [
...
def validate_static_config(config: dict[str, Any]) -> None:
    ...
    _expect(
        ...
        {
            "schema_version": "route_a_v3_gse200304_gse149487_public_evidence_runtime_sync.v1",
            "protocol_id": "ROUTE_A_V3_GSE200304_GSE149487_PUBLIC_EVIDENCE_RUNTIME_SYNC_V1",
            "contract_id": "mrna_xeditflow_route_a_v3",
            "phase_id": "A1",
            "dataset_ids": ["GSE200304", "GSE149487"],
            "decision_id": "V3-DEC-019",
            "event_id": "A1-EVT-045",
            "event_name": (
                "GSE200304_GSE149487_PUBLIC_EVIDENCE_REGISTERED_"
    ...
    _expect(
        ledger,
        {
            ...
            "manifest_status": (
                "A1_POST_FAIL_ACQUISITION_LEDGER_REGISTERED_PENDING_EVT045"
            ),
            "registered_lineage_ids": LEDGER_LINEAGE_IDS,
            "exact_changed_paths": [item["path"] for item in LEDGER_BLOBS],
            "frozen_blobs": LEDGER_BLOBS,
        },
        label="frozen predecessor ledger authority",
    )

    registered = config["registered_artifacts"]
    _expect(registered, REGISTERED_ARTIFACTS, label="ledger-ordered exact2 artifacts")
    if [item["lineage_id"] for item in registered] != ledger["registered_lineage_ids"]:
    ...
    _expect(
        config["registered_evidence_truth"],
        {
            ...
            "gse149487_public_asset_acquisition": {
        ...
        label="registered evidence truth",
    )
    _expect(
...
def validate_bound_config(config: dict[str, Any]) -> None:
    """Require the four implementation scalars before any external I/O."""
...
def validate_registered_artifacts(
    ...
    """Validate exact2 metadata and, in production, exact bytes without parsing."""
...
def _validate_recorded_at(recorded_at: str, predecessor_at: Any) -> None:
    ...
    if current.tzinfo is None or predecessor.tzinfo is None or current <= predecessor:
        raise PredecessorError("EVT045 timestamp must follow EVT044 with an explicit offset")
...
def validate_predecessor(
    ...
    """Validate EVT044 before creating any prepared member."""
    ...
    if len(events) != runtime["predecessor_event_count"]:
        raise PredecessorError("predecessor event count is not 44")
    tail = events[-1] if events else {}
    _expect(tail.get("event_id"), "A1-EVT-044", label="predecessor tail event")
    ...
    if not isinstance(outputs, list) or len(outputs) != runtime["predecessor_manifest_output_count"]:
        raise PredecessorError("predecessor manifest output count is not 181")
...
def _event_document(
    ...
    *,
    ...
    return {
        ...
        "decision_id": "V3-DEC-019",
        "predecessor_event_id": "A1-EVT-044",
        "registered_artifact_count": 2,
        ...
        "output_delta_count": 6,
        "manifest_output_count_before": 181,
        "manifest_output_count_after": 187,
        "new_runtime_output_count": 6,
        "scientific_state_changed": True,
        "evidence_surface_changed_since_evt044": True,
        "evidence_gate_statuses_changed_since_evt044": False,
        ...
        "next_phase_authorized": False,
        "detail": (
            "Registered the ledger-ordered exact2 public-evidence records in place, "
            "added three immutable EVT044 snapshots and one sync record. The evidence "
            "surface changed, but GSE200304 remains 7 PASS, 0 BLOCKED, 1 UNKNOWN, "
            "0 NOT_RUN with CHECKPOINT_SPECIFIC_EXPOSURE_NOT_PASS as its sole blocker; "
            "GSE149487 remains stopped and unqualified; outer A1 qualification and "
            "training/model-selection/next-phase authorization remain unchanged."
        ),
    }
...
def _build_sync_record(
    ...
    return json_bytes(
        {
            "record_type": "ROUTE_A_V3_A1_GSE200304_GSE149487_PUBLIC_EVIDENCE_RUNTIME_SYNC",
            "event_id": "A1-EVT-045",
            "decision_id": "V3-DEC-019",
...
def _immutable_outputs(
    ...
    # EVT044 already has 181 outputs. EVT045 appends the ledger-ordered exact2
    # public-evidence records, then three snapshots and one sync record, for 187.
...
def build_successors(
    ...
    # output list itself.
    successor_manifest["registered_artifact_count"] = 2
...
def validate_successors(
    ...
    """Check exact3 mutable prefix, exact2+4 output closure, and EVT045 last."""
    ...
    if len(events) != runtime["successor_event_count"] or events[:-1] != old_events:
        raise RuntimeSyncError("EVT045 is not one append-only event")
    event = events[-1]
    _expect(event.get("event_id"), "A1-EVT-045", label="successor event")
    ...
    _expect(
        event.get("evidence_surface_changed_since_evt044"),
        True,
        label="event evidence surface change",
    )
    _expect(
        event.get("evidence_gate_statuses_changed_since_evt044"),
        False,
    ...
    if not isinstance(event.get("at"), str):
        raise RuntimeSyncError("EVT045 timestamp is absent")
    outputs = manifest.get("outputs")
    if not isinstance(outputs, list) or len(outputs) != runtime["successor_manifest_output_count"]:
        raise RuntimeSyncError("successor manifest output count is not 187")
    ...
    if outputs[registered_start : registered_start + 2] != expected_registered:
        raise RuntimeSyncError("registered exact2 output metadata drift")
    ...
    if len({item.get("absolute_path") for item in outputs}) != 187:
    ...
    expected_manifest["registered_artifact_count"] = 2
    ...
    _expect(sync.get("event_id"), "A1-EVT-045", label="sync event id")
    _expect(sync.get("registered_artifact_count"), 2, label="sync exact2 count")
    _expect(sync.get("registered_artifacts"), expected_registered, label="sync exact2 metadata")
    ...
    _expect(
        sync.get("evidence_surface_changed_since_evt044"),
        True,
        label="sync evidence surface change",
    )
    _expect(
        sync.get("evidence_gate_statuses_changed_since_evt044"),
        False,
...
def _published_state_is_exact(config: dict[str, Any], payloads: Mapping[str, bytes]) -> bool:
    ...
    outputs = manifest.get("outputs")
    return (
        events[-1].get("event_id") == "A1-EVT-045"
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if _published_state_is_exact(config, predecessor):
            return {"status": "ALREADY_PUBLISHED_VERIFIED", "event_id": "A1-EVT-045"}
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        try:
            ...
            raise PublicationError(
                "EVT045 was not committed; retry with the same prepared directory"
            ) from exc
        final = _read_runtime(run_root)
        if final != successor:
            raise PublicationError("EVT045 publication finished with non-exact mutables")
        return {
            "status": "PUBLISHED_VERIFIED",
            "event_id": "A1-EVT-045",
            "reused": False,
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if current != successor:
            raise PublicationError("runtime does not exactly match prepared EVT045 successor")
        for name in config["runtime"]["immutable_publish_order"]:
    ...
    return {"status": "PUBLISHED_VERIFIED", "event_id": "A1-EVT-045"}
