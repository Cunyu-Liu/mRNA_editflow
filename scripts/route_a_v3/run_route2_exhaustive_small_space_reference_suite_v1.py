...
def validate_inputs(
    ...
    _require(isinstance(device, str) and device.startswith("cuda:"), "exhaustive execution requires CUDA")
    _require(isinstance(physical_gpu_index, int) and not isinstance(physical_gpu_index, bool) and physical_gpu_index >= 0, "invalid physical GPU index")
    _require(device == f"cuda:{physical_gpu_index}", "execution device differs from physical GPU index")
    _require(scoring.get("device") == device, "candidate generation and evaluator scoring device changed")
    _require(scoring.get("physical_gpu_index") == physical_gpu_index, "candidate generation and evaluator scoring physical GPU changed")
