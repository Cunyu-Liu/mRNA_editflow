#!/usr/bin/env python3
"""Evaluate one terminal SetFlow V4 checkpoint under the frozen 891x32 protocol."""
...
class SetFlowCheckpointValidationV4Error(RuntimeError):
    pass


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise SetFlowCheckpointValidationV4Error(message)
...
def require_screen_training_package_terminal_v4(
    ...
    for required_run in ("v4_full", "v4_single_mode"):
        ...
        _require(
            ...
            f"SetFlow V4 training package has a technical failure: {required_run}",
        )
        ...
        _require(
            ...
            == {"4", "6", "8", "10"},
            f"SetFlow V4 checkpoint package is incomplete: {required_run}",
        )
...
def load_checkpoint_v4(
    ...
    *,
    ...
    _require(
        ...
        == "TERMINAL_XEDITSETFLOW_V4_TRAINING_COMPLETE_PENDING_VALIDATION",
        "SetFlow V4 training is not terminal before checkpoint validation",
    )
    _require(
        ...
        "SetFlow V4 training read Validation generation or selected early",
    )
    _require(
        set(training_summary.get("saved_checkpoint_paths", {}))
        == {"4", "6", "8", "10"},
        "SetFlow V4 terminal checkpoint package is incomplete",
    )
    ...
    _require(
        ...
        "SetFlow V4 checkpoint path differs from the frozen run directory",
    )
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
    _require(
        checkpoint.get("schema_version")
        == "route_a_v3_route2_xeditsetflow_v4_checkpoint.v1",
        "unexpected SetFlow V4 checkpoint schema",
    )
    _require(
    ...
    _require(capacity == checkpoint["capacity"], "SetFlow V4 checkpoint capacity changed")
...
@torch.no_grad()
def evaluate_common_validation_nll_v4(
    ...
    *,
    ...
    _require(total_weight > 0.0 and active_count > 0, "SetFlow V4 common Validation set is empty")
    return {
...
def small_graph_exact_check_v4(
    model,
    checkpoint: Mapping[str, Any],
    device: torch.device,
) -> dict[str, Any]:
    ...
    _require(total_variation <= 1e-12, "SetFlow V4 small-graph mixture differs from enumeration")
    return {
        "source_length": 2,
        "edit_budget": 2,
...
def validate_checkpoint(
    ...
    *,
    ...
    _require(
        ...
        "SetFlow V4 validation GPU is outside 0–5",
    )
    ...
    model, checkpoint, training_summary = load_checkpoint_v4(
        config,
        run_id=run_id,
    ...
    started = time.time()
    common_nll = evaluate_common_validation_nll_v4(
        model,
        validation_rows,
        checkpoint,
        source_cache=cache,
    ...
    _require(
        ...
        "SetFlow V4 generation source cohort changed",
    )
    _require(
        all(
        ...
        "SetFlow V4 candidate cap changed",
    )
    ...
    for source_index, (root, metadata, prior) in enumerate(
        ...
        _require(len(allocated_modes) == 32, "SetFlow V4 source trajectory budget changed")
    ...
    _require(len(roots) == 891 * 32, "SetFlow V4 trajectory count changed")
    ...
    small_graph = small_graph_exact_check_v4(model, checkpoint, device)
