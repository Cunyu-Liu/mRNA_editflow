...
def load_task_records(canonical_path: Path, manifest_path: Path) -> tuple[list[TaskRecord], list[dict[str, Any]]]:
    ...
    _require(selected_manifest, "GSE114002 is absent from Development manifest")
    _require(all(row["split"] in SPLITS for row in selected_manifest.values()), "task manifest split changed")
    records = []
    seen: set[str] = set()
    ...
    _require(all(any(record.split == split for record in records) for split in SPLITS), "task split is incomplete")
...
def _train_multimolecule_rnafm_probe(
    ...
    seed: int, epochs: int, learning_rate: float, weight_decay: float,
    ...
    mean, std = features["TRAIN"].mean(dim=0), features["TRAIN"].std(dim=0).clamp_min(1e-6)
    ...
    weights = _source_group_weights(train, device)
    ...
    for epoch in range(epochs):
        ...
        prediction = head(features["TRAIN"]).squeeze(1)
        loss = ((prediction - targets["TRAIN"]) ** 2 * weights).mean()
...
def execute(config: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    records, task_manifest = load_task_records(Path(config["canonical_path"]), Path(config["development_manifest_path"]))
    ...
    try:
        ...
        for baseline_id, provenance_id, model in methods:
            ...
            method_dir = temporary / baseline_id; method_dir.mkdir()
            for split in ("VALIDATION", "TEST"):
                _write_predictions(method_dir / f"{split.lower()}_predictions.jsonl", baseline_id, records, predictions, split)
            summaries[baseline_id] = {
                ...
                "validation": _metrics(records, predictions, "VALIDATION"),
                "test": _metrics(records, predictions, "TEST"),
            }
            del model
        embeddings, rnafm_parameter_count = _multimolecule_rnafm_embeddings(
            ...
            Path(config["rnafm_model_path"]), device, int(config["rnafm_batch_size"]),
        )
        predictions, probe = _train_multimolecule_rnafm_probe(
            records, embeddings, device, int(config["seed"]), int(config["rnafm_probe_epochs"]),
            float(config["rnafm_probe_learning_rate"]), float(config["rnafm_probe_weight_decay"]),
        )
        method_dir = temporary / "multimolecule_rnafm_frozen_linear_probe"; method_dir.mkdir()
        for split in ("VALIDATION", "TEST"):
            _write_predictions(method_dir / f"{split.lower()}_predictions.jsonl", "multimolecule_rnafm_frozen_linear_probe", records, predictions, split)
