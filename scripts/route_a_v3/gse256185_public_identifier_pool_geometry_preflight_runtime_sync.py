#!/usr/bin/env python3
"""Prepare and publish the append-only A1 GSE256185 aggregate preflight sync.
...
to RUN_MANIFEST, snapshots the three EVT052 runtime mutables, writes one sync
record, then commits STATUS, RUN_MANIFEST, and EVENT_LOG in that order.  The
EVT053 event is therefore the commit point.

The candidate config permits a grouped UNKNOWN ledger bundle and a grouped
UNKNOWN implementation bundle for staging.  Every production entry point
requires both bundles to be bound and proves the exact L -> exact3 I ->
config-only B Git chain before reading the report or runtime.
"""
...
SCRIPT_REPO_PATH = (
    "scripts/route_a_v3/"
    "gse256185_public_identifier_pool_geometry_preflight_runtime_sync.py"
)
TEST_REPO_PATH = (
    "tests/route_a_v3/"
    "test_gse256185_public_identifier_pool_geometry_preflight_runtime_sync.py"
)
CONFIG_REPO_PATH = (
    "configs/"
    "route_a_v3_gse256185_public_identifier_pool_geometry_preflight_runtime_sync_v1.json"
)
...
LINEAGE_ID = "gse256185_public_identifier_pool_geometry_preflight_v1"
REPORT_NAME = "GSE256185_PUBLIC_IDENTIFIER_POOL_GEOMETRY_PREFLIGHT_V1.json"

FaultInjector = Callable[[str], None]


class RuntimeSyncError(RuntimeError):
    """Base error for the EVT053 publisher."""
...
class AuthorityError(RuntimeSyncError):
    """The production Git lineage is not the frozen L -> I -> B chain."""


class PredecessorError(RuntimeSyncError):
    """The runtime is neither exact EVT052 nor a supported EVT053 prefix."""
...
def _expected_report() -> dict[str, Any]:
    return {
        ...
        "name": REPORT_NAME,
        "artifact_type": (
            "PUBLIC_IDENTIFIER_AND_POOL_GEOMETRY_PREFLIGHT_AGGREGATE_ONLY"
        ),
        "absolute_path": (
...
def _expected_evidence_truth() -> dict[str, Any]:
    return {
        ...
        "status": "PREFLIGHT_COMPLETE_AGGREGATE_GEOMETRY_ONLY_NOT_QUALIFIED",
...
def validate_static_config(config: dict[str, Any]) -> None:
    ...
    _expect(
        ...
        {
            "schema_version": (
                "route_a_v3_gse256185_public_identifier_pool_geometry_"
            ...
            "event_id": "A1-EVT-053",
            "event_name": (
                "GSE256185_PUBLIC_IDENTIFIER_POOL_GEOMETRY_PREFLIGHT_"
    ...
    _expect(
        ...
        "LEDGER_L_EXACT4_THEN_IMPLEMENTATION_I_EXACT3_THEN_CONFIG_ONLY_B_V1",
    ...
    _expect(
        ...
        },
        {
            ...
            "manifest_status": (
                "A1_GSE256185_PUBLIC_IDENTIFIER_POOL_GEOMETRY_PREFLIGHT_"
                "LEDGER_REGISTERED_PENDING_EVT053"
            ),
    ...
    _expect(runtime["predecessor_event_id"], "A1-EVT-052", label="predecessor event")
    _expect(runtime["successor_event_id"], "A1-EVT-053", label="successor event")
    for key, expected in (
        ("predecessor_event_count", 52),
        ("successor_event_count", 53),
        ("predecessor_manifest_output_count", 224),
        ("successor_manifest_output_count", 229),
    ...
    _expect(tail["event_id"], "A1-EVT-052", label="tail event")
    _expect(tail["decision_id"], "V3-DEC-021", label="tail decision")
    ...
    _expect(
        runtime["sync_name"],
        "A1_GSE256185_PUBLIC_IDENTIFIER_POOL_GEOMETRY_PREFLIGHT_RUNTIME_SYNC_V1.json",
        label="sync name",
    )
...
def audit_production_repository_authority(
    ...
    """Prove exact L -> exact3 I -> config-only B before external I/O."""
    ...
    for item in ledger["frozen_blobs"]:
        path = item["path"]
        digest = item["sha256"]
        for commit, label in ((l_commit, "L"), (head, "B")):
...
def validate_predecessor(
    ...
    if len(events) != 52:
        raise PredecessorError("predecessor event count is not 52")
    _expect(events[-1].get("event_id"), "A1-EVT-052", label="tail event")
    _expect(events[-1].get("decision_id"), "V3-DEC-021", label="tail decision")
    outputs = manifest.get("outputs")
    if not isinstance(outputs, list) or len(outputs) != 224:
        raise PredecessorError("predecessor manifest output count is not 224")
    _expect(
...
def _validate_recorded_at(recorded_at: str, predecessor_at: Any) -> None:
    ...
    if current.tzinfo is None or predecessor.tzinfo is None or current <= predecessor:
        raise PredecessorError("EVT053 timestamp must follow EVT052 with an offset")
...
def _sync_record(
    ...
    return json_bytes(
        {
            "record_type": (
                "ROUTE_A_V3_A1_GSE256185_PUBLIC_IDENTIFIER_POOL_GEOMETRY_"
                "PREFLIGHT_RUNTIME_SYNC"
            ),
            "event_id": "A1-EVT-053",
            "decision_id": "V3-DEC-021",
            "recorded_at": recorded_at,
            "predecessor_event_id": "A1-EVT-052",
            "registered_artifact_count": 1,
...
def _event_document(
    ...
    return {
        ...
        "output_delta_count": 5,
        "manifest_output_count_before": 224,
        "manifest_output_count_after": 229,
        ...
        "scientific_claim_status": "NOT_ESTABLISHED",
        "detail": (
            "Registered one in-place GSE256185 ordinary-public aggregate pool-"
            "geometry report after exact-byte validation without parsing its body. "
            "The evidence surface changed, while GSE256185 remains unqualified with "
...
def build_successors(
    ...
    successor_manifest["registered_artifact_count"] = 5
...
def validate_successors(
    ...
    if len(events) != 53 or events[:-1] != old_events:
        raise RuntimeSyncError("EVT053 is not one append-only event")
    event = events[-1]
    _expect(event.get("event_id"), "A1-EVT-053", label="successor event")
    ...
    if not isinstance(outputs, list) or len(outputs) != 229:
        raise RuntimeSyncError("successor manifest output count is not 229")
    if outputs[:224] != old_manifest["outputs"]:
        raise RuntimeSyncError("predecessor manifest output prefix drift")
    if outputs[224] != config["registered_artifacts"][0]:
    ...
    if len({item.get("absolute_path") for item in outputs}) != 229:
...
def _published_state_is_evt053(
    config: dict[str, Any], payloads: Mapping[str, bytes]
) -> bool:
    try:
        status, manifest, events = _parse_runtime(payloads)
    ...
    return (
        len(events) == 53
        and events[-1].get("event_id") == "A1-EVT-053"
        and events[-1].get("decision_id") == "V3-DEC-021"
        and events[-1].get("scientific_state_changed") is False
        and isinstance(outputs, list)
        and len(outputs) == 229
        and manifest.get("registered_artifact_count") == 5
...
def prepare_runtime_sync(
    *,
    ...
    with _locked_run(run_root):
        ...
        if _published_state_is_evt053(config, predecessor):
            return {"status": "ALREADY_PUBLISHED_VERIFIED", "event_id": "A1-EVT-053"}
    ...
    return {
        ...
        "manifest_output_transition": "224_TO_229",
        "manifest_registered_artifact_transition": "4_TO_5",
        "registered_artifact_body_parse_count": 0,
    }
    ...
    with _locked_run(run_root):
        ...
        try:
            ...
            raise PublicationError(
                "EVT053 was not committed; retry the same prepared directory"
            ) from exc
        final = _read_runtime(run_root)
        if final != successor:
            raise PublicationError("EVT053 publication ended with non-exact mutables")
        return {
            "status": "PUBLISHED_VERIFIED",
            "event_id": "A1-EVT-053",
            "reused": False,
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if current != successor:
            raise PublicationError("runtime does not match the prepared EVT053 successor")
        for name in config["runtime"]["immutable_publish_order"]:
