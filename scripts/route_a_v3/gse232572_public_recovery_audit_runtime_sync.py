#!/usr/bin/env python3
"""Prepare, validate, and publish the A1-EVT-047 runtime sync.

EVT047 registers the existing GSE232572 public-recovery audit aggregate. A
production prepare verifies its exact bytes and SHA-256 identity without
parsing the JSON body. The only other immutable outputs are the three
...
The frozen I1 candidate is the direct exact3 child of L and leaves the four
implementation-binding scalars UNKNOWN_NOT_ASSERTED.  This append-only I2
changes only the publisher and its focused test.  Config-only B2 then binds I2.
Production proves L -> frozen exact3 I1 -> exact2 I2 -> config-only B2, clean
HEAD/upstream/origin equality, and frozen blobs before report, prepared-output,
or runtime I/O.
"""
...
SCRIPT_REPO_PATH = "scripts/route_a_v3/gse232572_public_recovery_audit_runtime_sync.py"
TEST_REPO_PATH = "tests/route_a_v3/test_gse232572_public_recovery_audit_runtime_sync.py"
CONFIG_REPO_PATH = "configs/route_a_v3_gse232572_public_recovery_audit_runtime_sync_v1.json"
FROZEN_I1_COMMIT = "c1b607a70333421d85e90658e56f21160d695f25"
FROZEN_I1_BLOB_SHA256 = {
    CONFIG_REPO_PATH: "dc1762a9f094b985135fda02966ee5a85fa26098f2664a6d571eafd290b757fc",
    SCRIPT_REPO_PATH: "bfe8b2ad399968aa4550df842d8d3125deb05e2cfa651dd29759d95a2b39e077",
    TEST_REPO_PATH: "02b51d1c4e75c76ca0f6f1473618289f4346b0ae78e34228977998fd9d9b426a",
}
...
REGISTERED_ARTIFACTS = [
    {
        ...
        "name": "GSE232572_A1_RECOVERY_REPORT.json",
...
class RuntimeSyncError(RuntimeError):
    """Base class for an EVT047 validation/publication error."""
...
class PredecessorError(RuntimeSyncError):
    """The runtime is not the exact EVT046 predecessor or a supported prefix."""
...
def _ledger_values(ledger: Mapping[str, Any]) -> list[Any]:
    ...
    lineage_id = (
        lineage_ids[0]
        if isinstance(lineage_ids, list) and len(lineage_ids) == 1
        else None
    )
    return [
        ledger.get("status"),
...
def _validate_ledger_binding(ledger: Mapping[str, Any]) -> None:
    """Allow only the grouped UNKNOWN candidate or one fully bound L bundle."""
...
def _validate_runtime_shape(config: dict[str, Any]) -> None:
    ...
    _expect(runtime["predecessor_event_id"], "A1-EVT-046", label="predecessor event id")
    _expect(runtime["successor_event_id"], "A1-EVT-047", label="successor event id")
    for key, expected in (
        ("predecessor_event_count", 46),
        ("successor_event_count", 47),
        ("predecessor_manifest_output_count", 192),
        ("successor_manifest_output_count", 197),
    ...
    _expect(tail["event_id"], "A1-EVT-046", label="predecessor tail event")
    ...
    _expect(
        runtime["sync_name"],
        "A1_GSE232572_PUBLIC_RECOVERY_AUDIT_RUNTIME_SYNC_V1.json",
        label="sync name",
    )
    immutable_expected = [
...
def validate_static_config(config: dict[str, Any]) -> None:
    ...
    _expect(
        ...
        {
            "schema_version": "route_a_v3_gse232572_public_recovery_audit_runtime_sync.v1",
            "protocol_id": "ROUTE_A_V3_GSE232572_PUBLIC_RECOVERY_AUDIT_RUNTIME_SYNC_V1",
    ...
    _expect(
        ledger["exact_changed_paths"],
        LEDGER_PATHS,
        label="predecessor ledger exact4 paths",
    )
    ...
    _expect(
        config["registered_evidence_truth"],
        {
            ...
            "gse217518_public_authority_preflight": {
                "schema_version": (
                    "route_a_v3_gse217518_public_authority_preflight.v1"
                ),
            ...
            "gse232572_public_recovery_audit": {
                "schema_version": "1.0.0",
                "record_type": (
                    "GSE232572_PUBLIC_RECOVERY_AUDIT_AGGREGATE_ONLY"
                ),
                "status": (
...
def audit_production_repository_authority(
    ...
    """Prove the pushed, clean L -> frozen I1 -> exact2 I2 -> B2 chain."""
...
def _validate_recorded_at(recorded_at: str, predecessor_at: Any) -> None:
    ...
    if current.tzinfo is None or predecessor.tzinfo is None or current <= predecessor:
        raise PredecessorError("EVT047 timestamp must follow EVT046 with an explicit offset")
...
def validate_predecessor(
    ...
    """Validate EVT046 before creating any prepared member."""
    ...
    if len(events) != runtime["predecessor_event_count"]:
        raise PredecessorError("predecessor event count is not 46")
    tail = events[-1] if events else {}
    _expect(tail.get("event_id"), "A1-EVT-046", label="predecessor tail event")
    ...
    if not isinstance(outputs, list) or len(outputs) != runtime["predecessor_manifest_output_count"]:
        raise PredecessorError("predecessor manifest output count is not 192")
...
def _event_document(
    ...
    *,
    ...
    return {
        ...
        "decision_id": "V3-DEC-019",
        "predecessor_event_id": "A1-EVT-046",
        "registered_artifact_count": 1,
        ...
        "output_delta_count": 5,
        "manifest_output_count_before": 192,
        "manifest_output_count_after": 197,
        "new_runtime_output_count": 5,
        "scientific_state_changed": True,
        "evidence_surface_changed_since_evt046": True,
        "evidence_gate_statuses_changed_since_evt046": False,
        ...
        "next_phase_authorized": False,
        "detail": (
            "Registered the exact GSE232572 public-recovery audit aggregate in place, "
            "added three immutable EVT046 snapshots and one sync record. The evidence "
            ...
            "GSE149487 remains stopped and unqualified; GSE217518 remains stopped "
            "before ordinary-public row-level production; GSE232572 now has 8,068 "
            "private development reconstructions from 11,929 published rows, with "
            "3,861 rejected rows, but remains AUDIT_PENDING and unqualified. No private "
            "JSONL is registered. Outer A1 qualification and "
...
def _build_sync_record(
    ...
    return json_bytes(
        {
            "record_type": "ROUTE_A_V3_A1_GSE232572_PUBLIC_RECOVERY_AUDIT_RUNTIME_SYNC",
            "event_id": "A1-EVT-047",
            "decision_id": "V3-DEC-019",
...
def _immutable_outputs(
    ...
    # EVT046 already has 192 outputs. EVT047 appends the exact1 GSE232572 report,
    # then three snapshots and one sync record, for 197.
...
def validate_successors(
    ...
    """Check exact3 mutable prefix, exact1+4 output closure, and EVT047 last."""
    ...
    if len(events) != runtime["successor_event_count"] or events[:-1] != old_events:
        raise RuntimeSyncError("EVT047 is not one append-only event")
    event = events[-1]
    _expect(event.get("event_id"), "A1-EVT-047", label="successor event")
    ...
    _expect(
        event.get("evidence_surface_changed_since_evt046"),
        True,
        label="event evidence surface change",
    )
    _expect(
        event.get("evidence_gate_statuses_changed_since_evt046"),
        False,
    ...
    if not isinstance(event.get("at"), str):
        raise RuntimeSyncError("EVT047 timestamp is absent")
    outputs = manifest.get("outputs")
    if not isinstance(outputs, list) or len(outputs) != runtime["successor_manifest_output_count"]:
        raise RuntimeSyncError("successor manifest output count is not 197")
    ...
    if len({item.get("absolute_path") for item in outputs}) != 197:
    ...
    _expect(sync.get("event_id"), "A1-EVT-047", label="sync event id")
    ...
    _expect(
        sync.get("evidence_surface_changed_since_evt046"),
        True,
        label="sync evidence surface change",
    )
    _expect(
        sync.get("evidence_gate_statuses_changed_since_evt046"),
        False,
...
def _published_state_is_exact(config: dict[str, Any], payloads: Mapping[str, bytes]) -> bool:
    ...
    outputs = manifest.get("outputs")
    return (
        events[-1].get("event_id") == "A1-EVT-047"
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if _published_state_is_exact(config, predecessor):
            return {"status": "ALREADY_PUBLISHED_VERIFIED", "event_id": "A1-EVT-047"}
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        try:
            ...
            raise PublicationError(
                "EVT047 was not committed; retry with the same prepared directory"
            ) from exc
        final = _read_runtime(run_root)
        if final != successor:
            raise PublicationError("EVT047 publication finished with non-exact mutables")
        return {
            "status": "PUBLISHED_VERIFIED",
            "event_id": "A1-EVT-047",
            "reused": False,
    ...
    *,
    ...
    with _locked_run(run_root):
        ...
        if current != successor:
            raise PublicationError("runtime does not exactly match prepared EVT047 successor")
        for name in config["runtime"]["immutable_publish_order"]:
    ...
    return {"status": "PUBLISHED_VERIFIED", "event_id": "A1-EVT-047"}
