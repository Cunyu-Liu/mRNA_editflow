#!/bin/bash
# P2-05: Sequential GRPO pilot launcher — REDUCED MEMORY CONFIG
#
# Previous runs OOM'd because each seed needs ~40 GB (32 trajectories with
# computation graphs × attention scores). This launcher uses:
#   --group-size 4  (was 8)  → K=4 degraded from spec K=8
#   --n-groups 2    (was 4)  → 2 groups per iteration
#   PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
#
# Total trajectories per iteration: 2×4=8 (was 4×8=32, 4x reduction).
# This is a DEGRADED run — documented in docs/p2_05_grpo_pilot.md.
#
# Usage:
#   nohup bash scripts/launch_p2_05_grpo_cds_reduced.sh > logs/p2_05_grpo_cds_reduced.nohup.log 2>&1 &
set -uo pipefail
...
for seed in ${SEEDS}; do
    ...
    echo "[$(date)] === Launching seed ${seed} on GPU ${GPU} (reduced config) ==="
