#!/bin/bash
# P2-05: GRPO pilot with --max-steps to control memory
#
# Memory math: n_groups × group_size × max_steps forward passes with gradients
# Each forward pass: ~366 MB (981 nt CDS, 8 heads, 6 layers)
# Config: 2 × 8 × 5 = 80 × 366 MB ≈ 29 GB (fits in 40 GB GPU)
#
# K=8 (spec-compliant), n_groups=2 (degraded from 4), max_steps=5 (degraded from 256)
...
for seed in ${SEEDS}; do
    ...
    echo "[$(date)] === Launching seed ${seed} on GPU ${GPU} (max-steps=5, K=8, n_groups=2) ==="
