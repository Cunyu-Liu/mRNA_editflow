...
def build(out_dir: Path, restricted_dir: Path, canonical_path: Path, config_hash: str):
    ...
    with open(canonical_path, "r", encoding="utf-8") as fh:
        for idx, line in enumerate(fh):
            ...
            if has_pair:
                ...
                counters["pairs"] += 1
                # group assignment: pair grouped by SOURCE
                gid = f"grp_src_{rid}"
                local_group_w.write(jl({
                    "group_id": gid,
                    "grouping_atom": "SOURCE",
                    "member_ids": [cand_id_f],
                    "group_sha256": sha256_str(str([cand_id_f])),
                }) + "\n")
                local_group_assign_w.write(jl({
                    "assignment_id": f"asg_{cand_id_f}",
                    "object_id": cand_id_f,
                    "object_type": "PAIR",
                    "group_id": gid,
                    "grouping_atom": "SOURCE",
                    "assignment_algorithm_id": "native_record_id",
                }) + "\n")
            else:
            ...
            for key, value in labels.items():
                ...
                bias += 1
                ctx_id = f"ctx_{acc_.lower()}" if (acc_ := acc) else "ctx_unknown"
                oc = {
                ...
                # group assignment: observation grouped by SEQUENCE
                gid = f"grp_seq_{cand_id if candidate else source_id}"
                local_group_w.write(jl({
                    "group_id": gid,
                    "grouping_atom": "SEQUENCE",
                    "member_ids": [obs_id],
                    "group_sha256": sha256_str(str([obs_id])),
                }) + "\n")
                local_group_assign_w.write(jl({
                    "assignment_id": f"asg_{obs_id}",
                    "object_id": obs_id,
    ...
    for w in (seq_w, obs_cand_w, obs_w, rel_cand_w, pair_w, use_w, group_w, group_assign_w,
              rej_w, trans_w, exp_w, eff_w,
              r_seq_w, r_obs_cand_w, r_obs_w, r_rel_cand_w, r_pair_w, r_use_w, r_group_w, r_group_assign_w,
              r_rej_w, r_trans_w, r_exp_w, r_eff_w):
        w.close()
    r_access_w.close()
